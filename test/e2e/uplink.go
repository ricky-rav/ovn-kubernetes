...
const (
	...
	// and far above the ARP noise on the segment.
...
var _ = ginkgo.Describe("Network Segmentation Uplink default-VRF egress", feature.NetworkSegmentation, feature.Uplink, func() {
	...
	ginkgo.It("programs and withdraws ECMP default routes for two Uplink gateways", func() {
		...
		rxBeforeA := externalContainerRxPackets(routerA.container, routerA.uplinkIface.InfName)
		rxBeforeB := externalContainerRxPackets(routerB.container, routerB.uplinkIface.InfName)
		for range uplinkECMPConnections {
			uplinkPodToHostnameAndExpect(pod, server.IPv4, serverName)
		}
		rxA := externalContainerRxPackets(routerA.container, routerA.uplinkIface.InfName) - rxBeforeA
		rxB := externalContainerRxPackets(routerB.container, routerB.uplinkIface.InfName) - rxBeforeB
		framework.Logf("Uplink gateways received %d (%s) and %d (%s) packets over %d connections",
			rxA, gatewayA, rxB, gatewayB, uplinkECMPConnections)
		gomega.Expect(rxA).To(gomega.BeNumerically(">=", uplinkECMPMinPackets),
			"expected egress connections through gateway %s", gatewayA)
		gomega.Expect(rxB).To(gomega.BeNumerically(">=", uplinkECMPMinPackets),
		...
		for nodeName := range nodeIfaces {
			for _, gateway := range gateways {
				if err := execNodeCommand(
					nodeName,
					"ip %s route del default via %s dev %s metric 50000 2>/dev/null || true",
...
// externalContainerRxPackets returns the packets received so far on the
// interface of the external container.
func externalContainerRxPackets(container infraapi.ExternalContainer, ifName string) uint64 {
	ginkgo.GinkgoHelper()

	out, err := infraprovider.Get().ExecExternalContainerCommand(container,
		[]string{"cat", "/sys/class/net/" + ifName + "/statistics/rx_packets"})
	gomega.Expect(err).NotTo(gomega.HaveOccurred())
	packets, err := strconv.ParseUint(strings.TrimSpace(out), 10, 64)
	gomega.Expect(err).NotTo(gomega.HaveOccurred(), "failed to parse rx_packets of %s on %s: %q", ifName, container.Name, out)
	return packets
}
