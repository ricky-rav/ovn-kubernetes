...
var _ = ginkgo.Describe("Network Segmentation Uplink route advertisements", feature.NetworkSegmentation, feature.RouteAdvertisements, func() {
	...
	ginkgo.It("preserves pre-existing Uplink interface routes across VRF enslavement and release", func(ctx ginkgo.SpecContext) {
		...
		for _, family := range ipFamilySet.UnsortedList() {
			...
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			gomega.Expect(configureUplinkBridgeStaticRoute(
				ictx,
				bridgeName,
				nodeNames,
...
// configureUplinkBridgeStaticRoute installs a static route on the Uplink
// bridge of the given nodes, mimicking routing state installed by another
// agent (e.g. a DHCP client) on the Uplink interface before OVN-Kubernetes
// enslaves it into a CUDN VRF.
func configureUplinkBridgeStaticRoute(
	ictx infraapi.Context,
	bridgeName string,
	nodeNames []string,
	cidr string,
	...
	for _, nodeName := range nodeNames {
		// metric 50000 keeps the route from clobbering the node's own
		// default route on its management interface
		if err := runNodeCommand(
			nodeName,
			"ip %sroute replace %s via %s dev %s metric 50000",
			family,
			cidr,
			via,
			bridgeName,
...
// routing table holds a working route toward the destination.
