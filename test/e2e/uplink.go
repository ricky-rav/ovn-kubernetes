...
const (
	...
	// uplinkECMPConnections TCP connections, each hashed independently onto
	// one of two equal-cost gateways, carry at least five forward packets
	// each: uplinkECMPMinPackets per gateway sits far below an even split
	// and far above the ARP and ND noise on the segment.
	uplinkECMPConnections = 40
	uplinkECMPMinPackets  = 10
	// uplinkMainTable selects the main routing table in helpers that take a
	// VRF name.
	uplinkMainTable = ""
)
...
var _ = ginkgo.Describe("Network Segmentation Uplink default-VRF egress", feature.NetworkSegmentation, feature.Uplink, func() {
	...
	ginkgo.It("publishes the heaviest next hops of the lowest-metric default routes", func(ctx ginkgo.SpecContext) {
		nodes, err := f.ClientSet.CoreV1().Nodes().List(ctx, metav1.ListOptions{})
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		uplinkAlloc, err := allocators.AllocateBGP(f, ictx)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		_, nodeIfaces := setupUplinkNetwork(ictx, nodes.Items, ipFamilySet,
			"upnet"+testSuffix, []string{uplinkAlloc.BGPPeerSubnet, uplinkAlloc.BGPPeerSubnet6})
		bridgeName := uplinkBridgeName("upecmp" + testSuffix)
		gomega.Expect(configureUplinkBridge(f, ictx, bridgeName, nodeIfaces)).To(gomega.Succeed())

		gateways := map[utilnet.IPFamily][]string{
			utilnet.IPv4: {"192.0.2.1", "192.0.2.2", "192.0.2.3", "192.0.2.4"},
			utilnet.IPv6: {"2001:db8::1", "2001:db8::2", "2001:db8::3", "2001:db8::4"},
		}
		// Per family: the preferred (metric 50000) route carries a heavy and a
		// light next hop, the backup (metric 50001) route two more. Only the
		// heavy next hop of the preferred route must be published.
		var expectedGateways []string
		for family := range ipFamilySet {
			expectedGateways = append(expectedGateways, gateways[family][0])
		}
		ginkgo.By("installing unequal-weight and higher-metric default routes on the Uplink bridge")
		for _, node := range nodes.Items {
			for family := range ipFamilySet {
				...
				nextHops := gateways[family]
				// These routes test discovery only; deleting the test bridge removes them.
				_, err := execNodeCommand(node.Name,
					"%s route replace default metric 50000 nexthop via %s dev %s weight 4 onlink nexthop via %s dev %s weight 1 onlink",
					ipCmd, nextHops[0], bridgeName, nextHops[1], bridgeName)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				_, err = execNodeCommand(node.Name,
					"%s route replace default metric 50001 nexthop via %s dev %s weight 4 onlink nexthop via %s dev %s weight 1 onlink",
					ipCmd, nextHops[2], bridgeName, nextHops[3], bridgeName)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
			}
		}

		ginkgo.By("publishing only the heaviest next hops of the lowest-metric defaults")
		uplinkName := "uplink" + testSuffix
		createUplink(f, ictx, uplinkName, nodes.Items, nodeIfaces, bridgeName)
		waitForUplinkStatesResolved(f, uplinkName, bridgeName, nodes.Items)
		for _, node := range nodes.Items {
			...
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			actual, _, err := unstructured.NestedStringSlice(state.Object, "status", "defaultGateways")
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			gomega.Expect(actual).To(gomega.ConsistOf(expectedGateways))
		}
	})
	...
	ginkgo.It("runs a CUDN without Uplink default gateways and adds its default routes when they appear", func(ctx ginkgo.SpecContext) {
		...
		// The gateways the restored default routes will carry, per node and
		// family; the gateway router and the managed host VRF default route
		// must not exist before the restore and must appear after it.
		vrfName := waitForCUDNVRFName(f, networkName)
		type nodeGateway struct {
			node, cidr, gateway string
		}
		var nodeGateways []nodeGateway
		for _, pod := range []*corev1.Pod{clientPod, serverPod} {
			iface := nodeIfaces[pod.Spec.NodeName]
			for _, family := range ipFamilySet.UnsortedList() {
				gateway, ip, prefix := iface.IPv4Gateway, iface.IPv4, iface.IPv4Prefix
				if family == utilnet.IPv6 {
					gateway, ip, prefix = iface.IPv6Gateway, iface.IPv6, iface.IPv6Prefix
				}
				gateway, err = interfaceGateway(gateway, ip, prefix)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				gomega.Expect(gateway).NotTo(gomega.BeEmpty(),
					"expected a %s gateway for node %s", family, pod.Spec.NodeName)
				nodeGateways = append(nodeGateways, nodeGateway{pod.Spec.NodeName, defaultRouteCIDR(family), gateway})
			}
		}
		for _, ng := range nodeGateways {
			gomega.Expect(cudnGRHasRoute(f.ClientSet, networkName, ng.node, ng.cidr, ng.gateway)).To(gomega.BeFalse(),
				"expected no default route via %s on the gateway router of CUDN %s for node %s before the restore",
				ng.gateway, networkName, ng.node)
			gomega.Expect(uplinkRouteShownIn(ng.node, "vrf "+vrfName, ng.cidr, ng.gateway)).NotTo(gomega.Succeed(),
				"expected no default route via %s in host VRF %s on node %s before the restore",
				ng.gateway, vrfName, ng.node)
		}

		// Restore the default routes: discovery re-polls and publishes the
		// gateways, and each network is reprogrammed from the changed
		// UplinkState with its default routes.
		gomega.Expect(configureUplinkBridgeDefaultRoutes(
			ictx,
			bridgeName,
			nodeIfaces,
		...
		for _, ng := range nodeGateways {
			...
			gomega.Eventually(func() (bool, error) {
				return cudnGRHasRoute(f.ClientSet, networkName, ng.node, ng.cidr, ng.gateway)
				...
				networkName, ng.node, ng.gateway)
			gomega.Eventually(func() error {
				return uplinkRouteShownIn(ng.node, "vrf "+vrfName, ng.cidr, ng.gateway)
			}).WithTimeout(uplinkTimeout).WithPolling(uplinkPoll).Should(gomega.Succeed(),
				"expected the managed default route via %s in host VRF %s on node %s",
				ng.gateway, vrfName, ng.node)
		}
	...
	ginkgo.It("recreates an UplinkState deleted out of band", func() {
		...
		ginkgo.By("deleting the UplinkState out of band and waiting for a new one without restarting ovnkube-node")
		deleteUplinkStateAndWaitForRecreation(f, uplinkName, node.Name)
	...
	// Two default gateways on the Uplink bridge give the CUDN VRF one
	// multipath default route and the gateway router one static default
	// route per gateway; withdrawing a gateway drops it from both.
	ginkgo.It("programs and withdraws ECMP default routes for two Uplink gateways", func() {
		// Discovery publishes the next hop of each single-path default route
		// of the host interface, and status.defaultGateways holds at most
		// two entries. The kernel dumps two IPv6 default routes with the
		// same metric as one multipath route, so ECMP is exercised over IPv4.
		if !ipFamilySet.Has(utilnet.IPv4) {
			e2eskipper.Skipf("Uplink ECMP default gateways e2e requires IPv4")
		}
		ecmpFamily := utilnet.IPv4
		...
		ginkgo.By("provisioning the Uplink underlay with two gateway routers toward a server network")
		uplinkAlloc, err := allocators.AllocateBGP(f, ictx)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		uplinkNetwork, nodeIfaces := setupUplinkNetwork(
			ictx,
		...
		// The container runtime does not forward between its bridge
		// networks, so the server sits behind two router containers with a
		// leg on each network instead of behind the network gateway.
		serverNetwork, err := ictx.CreateNetwork("upecmpsrvnet"+testSuffix, uplinkAlloc.IPVRFSubnet)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		routerA := createUplinkRouterContainer(ictx, "upecmprta"+testSuffix, uplinkNetwork, serverNetwork)
		routerB := createUplinkRouterContainer(ictx, "upecmprtb"+testSuffix, uplinkNetwork, serverNetwork)
		gatewayA, gatewayB := routerA.uplinkIface.IPv4, routerB.uplinkIface.IPv4
		gomega.Expect(gatewayA).NotTo(gomega.BeEmpty())
		gomega.Expect(gatewayB).NotTo(gomega.BeEmpty())
		serverName := "upecmpsrv" + testSuffix
		server, err := ictx.CreateExternalContainer(infraapi.ExternalContainer{
			Name:    serverName,
			Image:   images.AgnHost(),
			CmdArgs: []string{"netexec"},
		...
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		// Replies take one router regardless of the one the request took.
		_, err = infraprovider.Get().ExecExternalContainerCommand(server,
			[]string{"ip", "route", "add", uplinkAlloc.BGPPeerSubnet, "via", routerA.serverIface.IPv4})
		gomega.Expect(err).NotTo(gomega.HaveOccurred())

		bridgeName := uplinkBridgeName("upecmp" + testSuffix)
		gomega.Expect(configureUplinkBridge(f, ictx, bridgeName, nodeIfaces)).To(gomega.Succeed())
		gomega.Expect(configureUplinkBridgeGatewayRoutes(ictx, bridgeName, nodeIfaces, gatewayA, gatewayB)).To(
			gomega.Succeed())

		uplinkName := "upecmp" + testSuffix
		createUplink(f, ictx, uplinkName, nodes.Items, nodeIfaces, bridgeName)
		waitForUplinkStatesResolved(f, uplinkName, bridgeName, nodes.Items)
		ginkgo.By("verifying both gateways are published on the UplinkStates")
		waitForUplinkStatesDefaultGatewaysConsistOf(f, uplinkName, nodes.Items, gatewayA, gatewayB)

		ginkgo.By("activating a CUDN on the Uplink")
		networkName := "upecmpnet" + testSuffix
		namespace := setupUplinkLayer3CUDN(f, ictx, ipFamilySet, networkName, uplinkName)
		...
		ginkgo.By("verifying the CUDN VRF and the gateway router hold a default route over both gateways")
		expectUplinkDefaultNextHops(f, networkName, node.Name, vrfName, ecmpFamily, gatewayA, gatewayB)

		ginkgo.By("verifying egress connections are spread over both gateways")
		// Count on the server-side leg of each router: only the packets a
		// router forwarded show up there, not the ones merely sent to it.
		txBeforeA := externalContainerTxPackets(routerA.container, routerA.serverIface.InfName)
		txBeforeB := externalContainerTxPackets(routerB.container, routerB.serverIface.InfName)
		for range uplinkECMPConnections {
			uplinkPodToHostnameAndExpect(pod, server.IPv4, serverName)
		}
		txA := externalContainerTxPackets(routerA.container, routerA.serverIface.InfName) - txBeforeA
		txB := externalContainerTxPackets(routerB.container, routerB.serverIface.InfName) - txBeforeB
		framework.Logf("Uplink gateways forwarded %d (%s) and %d (%s) packets over %d connections",
			txA, gatewayA, txB, gatewayB, uplinkECMPConnections)
		gomega.Expect(txA).To(gomega.BeNumerically(">=", uplinkECMPMinPackets),
			"expected egress connections through gateway %s", gatewayA)
		gomega.Expect(txB).To(gomega.BeNumerically(">=", uplinkECMPMinPackets),
			"expected egress connections through gateway %s", gatewayB)

		ginkgo.By("withdrawing the second gateway and rediscovering the Uplink on the pod's node")
		for nodeName := range nodeIfaces {
			gomega.Expect(withdrawUplinkBridgeGatewayRoute(nodeName, bridgeName, gatewayB)).To(gomega.Succeed())
		}
		// Discovery runs on UplinkState events, not on host route changes
		// (event-driven rediscovery is tracked in issue #6784).
		deleteUplinkStateAndWaitForRecreation(f, uplinkName, node.Name)
		waitForUplinkStatesResolved(f, uplinkName, bridgeName, []corev1.Node{node})
		waitForUplinkStatesDefaultGatewaysConsistOf(f, uplinkName, []corev1.Node{node}, gatewayA)
		waitForUplinkStateGatewayCondition(
			f,
			uplinkName,
			node.Name,
		...
		ginkgo.By("verifying the withdrawn gateway is removed from the CUDN VRF and the gateway router")
		expectUplinkDefaultNextHops(f, networkName, node.Name, vrfName, ecmpFamily, gatewayA)
		for _, family := range ipFamilySet.UnsortedList() {
			gomega.Expect(uplinkVRFUnreachableDefaultShown(node.Name, vrfName, family)).To(gomega.Succeed(),
				"expected the IPv%s unreachable default of VRF %s to survive the withdrawal", family, vrfName)
		}
		for range 5 {
			uplinkPodToHostnameAndExpect(pod, server.IPv4, serverName)
		}
...
var _ = ginkgo.Describe("Network Segmentation Uplink route advertisements", feature.NetworkSegmentation, feature.RouteAdvertisements, feature.Uplink, func() {
	...
	ginkgo.DescribeTable("preserves pre-existing Uplink interface routes across VRF enslavement and release", func(ctx ginkgo.SpecContext, nmStyleAddressing bool) {
		...
		for _, family := range ipFamilySet.UnsortedList() {
			...
			for _, node := range schedulableNodes.Items {
				node := node
				gomega.Eventually(func() error {
					return uplinkRouteShownIn(node.Name, "vrf "+networkName, defaultRouteCIDR(family), frrIP)
				}).WithTimeout(uplinkTimeout).WithPolling(uplinkPoll).Should(
					gomega.Succeed(),
		...
		gomega.Consistently(func() error {
			for _, family := range ipFamilySet.UnsortedList() {
				_, _, frrIP := preservedFor(family)
				for _, node := range schedulableNodes.Items {
					if err := uplinkRouteShownIn(node.Name, "vrf "+networkName, defaultRouteCIDR(family), frrIP); err != nil {
						return err
					}
				}
			}
		...
		for _, family := range ipFamilySet.UnsortedList() {
			...
			for _, node := range schedulableNodes.Items {
				node := node
				gomega.Eventually(func() error {
					return uplinkRouteShownIn(node.Name, "", defaultRouteCIDR(family), frrIP)
				}).WithTimeout(uplinkTimeout).WithPolling(uplinkPoll).Should(
					gomega.Succeed(),
	...
	ginkgo.It("discovers a default gateway that appears in the CUDN VRF after enslavement", func(ctx ginkgo.SpecContext) {
		if isDPUUplinkE2E() {
			e2eskipper.Skipf("full-mode Uplink bridge provisioning; the split DPU mode variant covers DPU")
		}
		nodes, err := f.ClientSet.CoreV1().Nodes().List(ctx, metav1.ListOptions{})
		...
		ginkgo.By("provisioning the Uplink bridge without default routes and creating the Uplink")
		bridgeName := uplinkBridgeName("uplate" + testSuffix)
		gomega.Expect(configureUplinkBridge(f, ictx, bridgeName, nodeIfaces)).To(gomega.Succeed())
		uplinkName := networkName
		createUplink(f, ictx, uplinkName, nodes.Items, nodeIfaces, bridgeName)
		waitForUplinkStatesResolved(f, uplinkName, bridgeName, nodes.Items)
		...
		ginkgo.By("waiting for the Uplink bridge to be enslaved to the CUDN VRF")
		for _, node := range schedulableNodes.Items {
			node := node
			gomega.Eventually(func() error {
				_, err := execNodeCommand(node.Name,
				...
				gomega.Succeed(),
				"expected Uplink bridge %s enslaved to VRF %s on node %s",
				bridgeName,
				networkName,
				node.Name,
			)
		...
		nodeNames := make([]string, 0, len(schedulableNodes.Items))
		for _, node := range schedulableNodes.Items {
			nodeNames = append(nodeNames, node.Name)
		}

		waitForUplinkStatesNoDefaultGateways(f, uplinkName, schedulableNodes.Items)
		...
		for _, family := range ipFamilySet.UnsortedList() {
			frrIP := getFirstIPStringOfFamily(family, []string{frrIface.IPv4, frrIface.IPv6})
			gomega.Expect(frrIP).NotTo(gomega.BeEmpty())
			hasDefaultRoute := func(nodeName string) (bool, error) {
				return cudnGRHasRoute(f.ClientSet, networkName, nodeName, defaultRouteCIDR(family), frrIP)
			}
			for _, node := range schedulableNodes.Items {
				gomega.Expect(hasDefaultRoute(node.Name)).To(gomega.BeFalse(),
					"expected no default route via %s on the gateway router of CUDN %s for node %s before discovery",
					frrIP, networkName, node.Name)
			}
			gomega.Expect(configureUplinkStaticRoute(
				ictx,
				bridgeName,
			...
			for _, node := range schedulableNodes.Items {
				gomega.Expect(uplinkRouteShownIn(node.Name, "vrf "+networkName, defaultRouteCIDR(family), frrIP)).
					To(gomega.Succeed(),
						"expected the added default route via %s in CUDN VRF %s on node %s",
						frrIP, networkName, node.Name)
				gomega.Eventually(func() (bool, error) { return hasDefaultRoute(node.Name) }).
					WithTimeout(uplinkTimeout).WithPolling(uplinkPoll).Should(gomega.BeTrue(),
					"expected CUDN %s gateway router on node %s to acquire default route via %s",
					networkName, node.Name, frrIP)
			}
		}

		ginkgo.By("waiting for discovery to publish the default gateways from the VRF routing table")
		waitForUplinkStatesDefaultGateways(f, uplinkName, schedulableNodes.Items, ipFamilySet)
		for _, node := range schedulableNodes.Items {
			waitForUplinkStateGatewayCondition(
				f,
...
var _ = ginkgo.Describe("Network Segmentation Uplink split DPU status conditions", feature.NetworkSegmentation, feature.Uplink, func() {
	...
	ginkgo.It("runs a CUDN without Uplink default gateways and publishes them when they appear", func(ctx ginkgo.SpecContext) {
		...
		// The network is rendered, and its gateway router created, only on
		// the node running a pod attached to it.
		podNode := dpuHostNodes[0]
		createUplinkNetexecPod(f, namespace.Name, "client-"+networkName, podNode.Name)
		waitForUplinkStateGatewayCondition(f, uplinkName, podNode.Name,
			metav1.ConditionTrue, uplinkv1alpha1.UplinkStateReasonGatewayConfigured)
		waitForUplinkStateConditionOfType(f, uplinkName, podNode.Name,
			uplinkv1alpha1.UplinkStateConditionHostGatewayReady,
			metav1.ConditionTrue, uplinkv1alpha1.UplinkStateReasonGatewayConfigured)
		waitForCUDNUplinksReady(f, networkName)

		ginkgo.By("adding default routes and waiting for the gateways to be published and programmed")
		for _, node := range dpuHostNodes {
			node := node
			gateways, err := addNodeInterfaceDefaultRoutes(ictx, node.Name, nodeIfaces[node.Name].InfName)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
...
// uplinkMasqueradeCIDRs are the OVN-Kubernetes masquerade subnets. ovnkube
// adds a host masquerade address from them to the Uplink gateway interface,
// which must not be mistaken for the interface's uplink subnet.
var uplinkMasqueradeCIDRs = []string{"169.254.0.0/17", "fd69::/112"}

// nodeInterfaceDefaultGateways derives a default gateway per IP family the
// node interface has a global address for, skipping the ovnkube masquerade
// addresses: the first usable IP of the address's subnet, or the next one
// when the interface holds the first usable IP itself. firstUsableIP does
// not apply here: it rejects the self-IP case that this helper supports.
func nodeInterfaceDefaultGateways(nodeName, ifName string) (map[utilnet.IPFamily]string, error) {
	masqueradeNets := make([]*net.IPNet, 0, len(uplinkMasqueradeCIDRs))
	for _, cidr := range uplinkMasqueradeCIDRs {
		_, ipNet, err := net.ParseCIDR(cidr)
		if err != nil {
			return nil, err
		}
		masqueradeNets = append(masqueradeNets, ipNet)
	}
	isMasquerade := func(ip net.IP) bool {
		for _, ipNet := range masqueradeNets {
	...
	for _, link := range links {
		for _, addr := range link.AddrInfo {
			...
			if _, found := gateways[family]; addr.Scope != "global" || found {
				continue
			}
			...
			gateways[family] = gatewayIP.String()
		}
	}
	return gateways, nil
}
...
// interface for each IP family it has a global address for, using the
// gateways derived by nodeInterfaceDefaultGateways, and returns them by
// family. A cleanup removing the routes is registered.
func addNodeInterfaceDefaultRoutes(
	ictx infraapi.Context,
	nodeName, ifName string,
) (map[utilnet.IPFamily]string, error) {
	gateways, err := nodeInterfaceDefaultGateways(nodeName, ifName)
	if err != nil {
		return nil, err
	}
...
// defaultRouteCIDR returns the default route destination of the IP family.
func defaultRouteCIDR(family utilnet.IPFamily) string {
	if family == utilnet.IPv6 {
		return "::/0"
	}
	return "0.0.0.0/0"
...
// cudnGRHasRoute reports whether the OVN gateway router of the CUDN on the
// node has a static route to cidr via nextHop.
func cudnGRHasRoute(k8sClient kubernetes.Interface, cudnName, nodeName, cidr, nextHop string) (bool, error) {
	routes, err := cudnGRRoutesForNode(k8sClient, cudnName, nodeName)
	if err != nil {
		return false, err
	}
	for _, line := range strings.Split(routes, "\n") {
		fields := strings.Fields(line)
		if len(fields) >= 2 && fields[0] == cidr && fields[1] == nextHop {
			return true, nil
		}
	}
	return false, nil
...
// configureUplinkBridgeGatewayRoutes appends one default route per gateway
// on the Uplink bridge of every node, all with the same metric so that the
// gateways are equal-cost, and registers their removal.
func configureUplinkBridgeGatewayRoutes(
	ictx infraapi.Context,
	bridgeName string,
	nodeIfaces map[string]infraapi.NetworkInterface,
	gateways ...string,
	...
	ictx.AddCleanUpFn(func() error {
		// Best effort: the spec may have withdrawn a route already, and
		// deleting the bridge removes the rest.
		for nodeName := range nodeIfaces {
			for _, gateway := range gateways {
				_ = withdrawUplinkBridgeGatewayRoute(nodeName, bridgeName, gateway)
			}
		}
		return nil
	})
	for nodeName := range nodeIfaces {
		for _, gateway := range gateways {
			if _, err := execNodeCommand(
				nodeName,
				"ip %s route append default via %s dev %s metric 50000",
				ipRouteFamilyFlag(utilnet.IPFamilyOfString(gateway)),
				gateway,
				bridgeName,
			); err != nil {
				return fmt.Errorf("failed to add default route via %s on %s/%s: %w", gateway, nodeName, bridgeName, err)
			}
		}
	}
	return nil
...
// withdrawUplinkBridgeGatewayRoute deletes the default route via the gateway
// added by configureUplinkBridgeGatewayRoutes on the node.
func withdrawUplinkBridgeGatewayRoute(nodeName, bridgeName, gateway string) error {
	_, err := execNodeCommand(
		nodeName,
		"ip %s route del default via %s dev %s metric 50000",
		ipRouteFamilyFlag(utilnet.IPFamilyOfString(gateway)),
		gateway,
		bridgeName,
	)
...
// (e.g. a DHCP client). An empty vrfName selects the main table; otherwise
// the route is installed in that VRF. With onlink the route is accepted
// regardless of the interface's addressing.
func configureUplinkStaticRoute(
	...
	tableSelector := "table main"
	if vrfName != "" {
		tableSelector = "vrf " + vrfName
	}
	ictx.AddCleanUpFn(func() error {
		var errs []error
		for _, nodeName := range nodeNames {
			...
			if _, err := execNodeCommand(
				nodeName,
				"ip %sroute del %s via %s dev %s %s metric 50000 2>/dev/null || true",
				family,
				cidr,
				via,
				devName,
	...
	for _, nodeName := range nodeNames {
		// Keep test default routes below the node's management default route.
		if _, err := execNodeCommand(
			nodeName,
			"ip %sroute replace %s via %s dev %s %s%s metric 50000",
			family,
			cidr,
			via,
			devName,
...
func configureUplinkNMStyleAddressing(ictx infraapi.Context, devByNode map[string]string) error {
	...
	reshape := func(nodeName, devName, flag, addrFilter, replayTolerance string) error {
		_, err := execNodeCommand(nodeName, `
set -e
dev=%[1]s
...
func nodeInterfacePrefixCIDR(nodeName, devName string, family utilnet.IPFamily) string {
	...
	familyFlag := ipRouteFamilyFlag(family)
	out, err := infraprovider.Get().ExecK8NodeCommand(nodeName, []string{
...
// ipRouteFamilyFlag returns the ip command flag selecting the IP family.
func ipRouteFamilyFlag(family utilnet.IPFamily) string {
	if family == utilnet.IPv6 {
		return "-6"
	}
	return "-4"
...
// uplinkVRFDefaultNextHops returns the next hops of the default route of the
// IP family in the VRF table on the node, expecting exactly one such route
// besides the unreachable default: a single-path route contributes its
// gateway, a multipath route all of its next hops.
func uplinkVRFDefaultNextHops(nodeName, vrfName string, family utilnet.IPFamily) ([]string, error) {
	out, err := infraprovider.Get().ExecK8NodeCommand(nodeName, []string{
		"ip", "-j", ipRouteFamilyFlag(family), "route", "show", "vrf", vrfName, "default",
	})
	if err != nil {
		return nil, fmt.Errorf("failed to list IPv%s default routes of VRF %s on node %s: %w", family, vrfName, nodeName, err)
	}
	var routes []kernelRoute
	if err := json.Unmarshal([]byte(out), &routes); err != nil {
		return nil, fmt.Errorf("failed to parse default routes of VRF %s on node %s: %w; output: %s",
			vrfName, nodeName, err, out)
	}
	var defaults []kernelRoute
	for _, route := range routes {
		if route.Gateway == "" && len(route.Nexthops) == 0 {
			continue
		}
		defaults = append(defaults, route)
	}
	if len(defaults) != 1 {
		return nil, fmt.Errorf("expected one IPv%s default route with next hops in VRF %s on node %s, got %d: %s",
			family, vrfName, nodeName, len(defaults), out)
	}
	if defaults[0].Gateway != "" {
		return []string{defaults[0].Gateway}, nil
	}
	nextHops := make([]string, 0, len(defaults[0].Nexthops))
	for _, nexthop := range defaults[0].Nexthops {
		nextHops = append(nextHops, nexthop.Gateway)
	}
	return nextHops, nil
}

// uplinkVRFUnreachableDefaultShown returns nil when the VRF table on the node
// holds the unreachable default route of the IP family.
func uplinkVRFUnreachableDefaultShown(nodeName, vrfName string, family utilnet.IPFamily) error {
	_, err := execNodeCommand(
		nodeName,
		"ip %s route show vrf %s type unreachable default | grep -q .",
		ipRouteFamilyFlag(family),
		vrfName,
	)
	return err
}

// cudnGRDefaultNextHops returns the next hops of the static default routes
// of the IP family on the OVN gateway router of the CUDN on the node.
func cudnGRDefaultNextHops(f *framework.Framework, cudnName, nodeName string, family utilnet.IPFamily) ([]string, error) {
	out, err := cudnGRRoutesForNode(f.ClientSet, cudnName, nodeName)
	if err != nil {
		return nil, err
	}
	prefix := defaultRouteCIDR(family)
	var nextHops []string
...
// expectUplinkDefaultNextHops waits until the default route of the IP family
// in the CUDN VRF on the node is a single route over exactly the given next
// hops and the OVN gateway router of the CUDN holds one static default route
// per next hop.
func expectUplinkDefaultNextHops(
	f *framework.Framework,
	cudnName, nodeName, vrfName string,
	...
	gomega.Eventually(func() ([]string, error) {
		return uplinkVRFDefaultNextHops(nodeName, vrfName, family)
	}).WithTimeout(uplinkTimeout).WithPolling(uplinkPoll).Should(gomega.ConsistOf(nextHops),
		"expected the IPv%s default route of VRF %s on node %s to go over %v", family, vrfName, nodeName, nextHops)
	gomega.Eventually(func() ([]string, error) {
		return cudnGRDefaultNextHops(f, cudnName, nodeName, family)
	}).WithTimeout(uplinkTimeout).WithPolling(uplinkPoll).Should(gomega.ConsistOf(nextHops),
		"expected the IPv%s default routes of the gateway router of %s on node %s to go over %v",
		family, cudnName, nodeName, nextHops)
}
...
// waitForUplinkStatesDefaultGatewaysConsistOf waits until the UplinkState of
// every node publishes exactly the given default gateways.
func waitForUplinkStatesDefaultGatewaysConsistOf(
	f *framework.Framework,
	uplinkName string,
	nodes []corev1.Node,
	gateways ...string,
	...
	for _, node := range nodes {
		gomega.Eventually(func() ([]string, error) {
			...
			defaultGateways, _, err := unstructured.NestedStringSlice(state.Object, "status", "defaultGateways")
			return defaultGateways, err
		}).WithTimeout(uplinkTimeout).WithPolling(uplinkPoll).Should(gomega.ConsistOf(gateways),
			"expected UplinkState for uplink %q on node %q to publish default gateways %v", uplinkName, node.Name, gateways)
	}
}

// deleteUplinkStateAndWaitForRecreation deletes the UplinkState of the node
// out of band and waits for the node to publish a new object, which reruns
// discovery of the host interface.
func deleteUplinkStateAndWaitForRecreation(f *framework.Framework, uplinkName, nodeName string) {
	ginkgo.GinkgoHelper()
...
// uplinkRouterContainer is a forwarding container with a leg on the Uplink
// network, where it serves as a default gateway of the nodes, and a leg on a
// server network behind it.
type uplinkRouterContainer struct {
	container   infraapi.ExternalContainer
	uplinkIface infraapi.NetworkInterface
	serverIface infraapi.NetworkInterface
}

func createUplinkRouterContainer(
	ictx infraapi.Context,
	name string,
	uplinkNetwork, serverNetwork infraapi.Network,
) uplinkRouterContainer {
	ginkgo.GinkgoHelper()

	container, err := ictx.CreateExternalContainer(infraapi.ExternalContainer{
		Name:    name,
		Image:   images.AgnHost(),
		Network: uplinkNetwork,
		RuntimeArgs: []string{
	...
	gomega.Expect(err).NotTo(gomega.HaveOccurred(), "expected to create router container %s", name)
	uplinkIface, err := infraprovider.Get().GetExternalContainerNetworkInterface(container, uplinkNetwork)
	gomega.Expect(err).NotTo(gomega.HaveOccurred())
	serverIface, err := ictx.AttachNetwork(serverNetwork, name)
	gomega.Expect(err).NotTo(gomega.HaveOccurred())
	return uplinkRouterContainer{
		container:   container,
...
// externalContainerTxPackets returns the packets sent so far on the
// interface of the external container.
func externalContainerTxPackets(container infraapi.ExternalContainer, ifName string) uint64 {
	ginkgo.GinkgoHelper()

	out, err := infraprovider.Get().ExecExternalContainerCommand(container,
		[]string{"cat", "/sys/class/net/" + ifName + "/statistics/tx_packets"})
	gomega.Expect(err).NotTo(gomega.HaveOccurred())
	packets, err := strconv.ParseUint(strings.TrimSpace(out), 10, 64)
	gomega.Expect(err).NotTo(gomega.HaveOccurred(), "failed to parse tx_packets of %s on %s: %q", ifName, container.Name, out)
	return packets
}
...
// dpuCUDNGatewayRouterHasDefaultRoute reports whether the OVN gateway router
// of the CUDN for the host node has a default static route via nextHop. The
// DPU's ovnkube programs that router in the DPU cluster, which the test has
// no API access to: the query runs through the DPU kind node container, with
// crictl, in the nb-ovsdb container of the DPU ovnkube pod.
func dpuCUDNGatewayRouterHasDefaultRoute(hostNodeName, cudnName, cidr, nextHop string) (bool, error) {
	dpuNodeName, err := dpuNodeNameForHostNode(hostNodeName)
	if err != nil {
		return false, err
	}
	out, err := ForContainer(dpuNodeName).Exec("crictl", "ps", "-q", "--name", "nb-ovsdb")
	if err != nil {
		return false, fmt.Errorf("failed to find the nb-ovsdb container on DPU node %s: %w, output: %s",
			dpuNodeName, err, out)
	}
	containerIDs := strings.Fields(out)
	if len(containerIDs) == 0 {
		return false, fmt.Errorf("no nb-ovsdb container running on DPU node %s", dpuNodeName)
	}
	out, err = ForContainer(dpuNodeName).Exec("crictl", "exec", containerIDs[0],
		"ovn-nbctl", "--bare", "--columns=nexthop", "find", "logical_router_static_route",
		"ip_prefix="+cidr,
		"external_ids:"+ovntypes.NetworkExternalID+"="+ovntypes.CUDNPrefix+cudnName)
	if err != nil {
		return false, fmt.Errorf("failed to list default routes of CUDN %s on DPU node %s: %w, output: %s",
			cudnName, dpuNodeName, err, out)
	}
	framework.Logf("Default routes %s of CUDN %s on DPU node %s for host node %s: %s",
		cidr, cudnName, dpuNodeName, hostNodeName, strings.TrimSpace(out))
	for _, programmed := range strings.Fields(out) {
		if programmed == nextHop {
			return true, nil
		}
	}
	return false, nil
