...
# The BGP/FRR helpers live in dedicated scripts so that external consumers can
# invoke them directly (see setup-bgp-peer.sh and setup-frr-k8s.sh); source
# them here to keep the kind.sh/kind-helm.sh flows unchanged.
KIND_COMMON_DIR="$( cd -- "$( dirname -- "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )"
source "${KIND_COMMON_DIR}/setup-frr-k8s.sh"
source "${KIND_COMMON_DIR}/setup-bgp-peer.sh"
