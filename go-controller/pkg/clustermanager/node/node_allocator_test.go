...
func TestController_allocateNodeSubnets_ReleaseOnError(t *testing.T) {
	...
	got, allocated, err := na.allocateNodeSubnets(na.clusterSubnetAllocator, "testNode", nil, nil, true, true)
	if err == nil {
...
func TestNodeAllocator_CleanupNodeWithoutNodeObject(t *testing.T) {
	...
	allocated, _, err := na.allocateNodeSubnets(na.clusterSubnetAllocator, nodeName, nil, nil, true, false)
	if err != nil {
	...
	if na.hasHybridOverlayAllocation() {
		if _, _, err := na.allocateNodeSubnets(na.hybridOverlaySubnetAllocator, nodeName, nil, nil, true, false); err != nil {
