...
func (na *NodeAllocator) hybridOverlayNodeEnsureSubnet(node *corev1.Node, annotator kube.Annotator) (*net.IPNet, error) {
	...
	hostSubnets, allocatedSubnets, err := na.allocateNodeSubnets(na.hybridOverlaySubnetAllocator, node.Name, node.GetLabels(), existingSubnets, true, false)
	if err != nil {
...
func (na *NodeAllocator) syncNodeNetworkAnnotations(node *corev1.Node) error {
	...
	if na.hasNodeSubnetAllocation() {
		...
		validExistingSubnets, allocatedSubnets, err = na.allocateNodeSubnets(na.clusterSubnetAllocator, node.Name, node.GetLabels(), existingSubnets, ipv4Mode, ipv6Mode)
		if err != nil {
			return err
		}
...
func (na *NodeAllocator) allocateNodeSubnets(allocator SubnetAllocator, nodeName string, nodeLabels map[string]string, existingSubnets []*net.IPNet, ipv4Mode, ipv6Mode bool) ([]*net.IPNet, []*net.IPNet, error) {
	...
	if hasNumPodsLabel {
		...
		if err != nil {
			return nil, nil, fmt.Errorf("incorrect setting of label %s on node %s, %v", ovnNodeNumPodsLabel, nodeName, err)
		}
		klog.V(5).Infof("Node %s requests %d IP addresses", nodeName, numPods)
	}

	// allocate new subnets if needed
	if ipv4Mode && !foundIPv4 {
		if err := allocateOneSubnet(allocator.AllocateIPv4Network(nodeName, numPods)); err != nil {
			return nil, nil, err
		}
	}
