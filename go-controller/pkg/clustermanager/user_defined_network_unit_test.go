	...
	ginkgo.Context("User-Defined Networks", func() {
		ginkgo.It("Attach layer3 UDN", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				netInfo, err := util.NewNetInfo(&ovncnitypes.NetConf{NetConf: cnitypes.NetConf{Name: "blue"}, Topology: ovntypes.Layer3Topology, Subnets: "192.168.0.0/16/24"}, nil)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
		...
		ginkgo.It("Cleanup", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				netInfo, err := util.NewNetInfo(&ovncnitypes.NetConf{NetConf: cnitypes.NetConf{Name: "blue"}, Topology: ovntypes.Layer3Topology}, nil)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
