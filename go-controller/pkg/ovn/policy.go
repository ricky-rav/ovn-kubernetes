...
type networkPolicy struct {
	...
	portGroupName string // without network prefix
...
func (oc *Controller) updateStaleDefaultDenyACLNames(npType knet.PolicyType, gressSuffix string) error {
	if util.IsClusterScoped() { // We return early if we cluster name is not empty.
		return nil
	}
	...
	gressACLs, err := libovsdbops.FindACLsWithPredicate(oc.mc.nbClient, p)
	if err != nil {
	...
	for namespace, aclList := range cleanUpDefaultDeny {
		...
		if len(aclList) > 1 {
			...
			ingressPGName := util.GetClusterScopedName(oc.nadInfo.Prefix + defaultDenyPortGroup(namespace, ingressDefaultDenySuffix))
			egressPGName := util.GetClusterScopedName(oc.nadInfo.Prefix + defaultDenyPortGroup(namespace, egressDefaultDenySuffix))
		...
		err := libovsdbops.CreateOrUpdateACLs(oc.mc.nbClient, aclList[0])
		if err != nil {
...
func (oc *Controller) syncNetworkPoliciesRetriableCommon(expectedPolicies map[string]map[string]bool) error {
	stalePGs := []string{}
	err := oc.addressSetFactory.ProcessEachAddressSet(func(addrSetName, namespaceName, policyName string) error {
		if policyName != "" && !expectedPolicies[namespaceName][policyName] {
			...
			hashedLocalPortGroup := util.GetClusterScopedName(oc.nadInfo.Prefix + hashedPortGroup(portGroupName))
	...
	egressACLs, err := libovsdbops.FindACLsWithPredicate(oc.mc.nbClient, p)
	if err != nil {
	...
	if len(egressACLs) > 0 {
		...
		ops, err := libovsdbops.CreateOrUpdateACLsOps(oc.mc.nbClient, nil, egressACLs...)
		if err != nil {
	...
	for _, gressACL := range gressACLs {
		...
		ops, err = libovsdbops.DeleteACLsOps(oc.mc.nbClient, ops, []string{pgName}, nil, gressACL)
		if err != nil {
...
func addAllowACLFromNode(nodeName string, mgmtPortIP net.IP, nbClient libovsdbclient.Client) error {
	...
	nodeACL := libovsdbops.BuildACL("", nbdb.ACLDirectionToLport, types.DefaultAllowPriority, match, "allow-related", "", "", false, externalIDs, nil)
...
func getACLMatch(portGroupName, match string, policyType knet.PolicyType, netNameInfo util.NetNameInfo) string {
	var aclMatch string
	portGroupName = util.GetClusterScopedName(netNameInfo.Prefix + portGroupName)
...
func namespacePortGroupACLName(namespace, portGroup, name string) string {
	...
	policyNamespace = util.GetClusterScopedName(policyNamespace)
	if name == "" {
...
// portGroupName is the portGroupName without network prefix
func buildACL(namespace, portGroup, name, direction string, priority int, match, action, aclLogging string, policyType knet.PolicyType, netNameInfo util.NetNameInfo) *nbdb.ACL {
	var options map[string]string
	// TBD: aclName's portGroup part does not have network prefix
...
func buildPortGroup(hashName, name string, ports []*nbdb.LogicalSwitchPort, acls []*nbdb.ACL, netNameInfo util.NetNameInfo) *nbdb.PortGroup {
	var externalIds map[string]string
	pgName := util.GetClusterScopedName(netNameInfo.Prefix + hashName)
...
func buildDenyACLs(namespace, policy, pg, aclLogging string, policyType knet.PolicyType, nadInfo *util.NetAttachDefInfo) (denyACL, allowACL *nbdb.ACL) {
	...
	allowMatch := getACLMatch(pg, "(arp || nd)", policyType, nadInfo.NetNameInfo)
	if policyType == knet.PolicyTypeIngress {
		denyACL = buildACL(namespace, pg, ingressDefaultDenySuffix, nbdb.ACLDirectionToLport, types.DefaultDenyPriority, denyMatch, nbdb.ACLActionDrop, aclLogging, policyType, nadInfo.NetNameInfo)
		allowACL = buildACL(namespace, pg, arpAllowPolicySuffix, nbdb.ACLDirectionToLport, types.DefaultAllowPriority, allowMatch, nbdb.ACLActionAllow, "", policyType, nadInfo.NetNameInfo)
	} else {
		denyACL = buildACL(namespace, pg, egressDefaultDenySuffix, nbdb.ACLDirectionFromLport, types.DefaultDenyPriority, denyMatch, nbdb.ACLActionDrop, aclLogging, policyType, nadInfo.NetNameInfo)
		allowACL = buildACL(namespace, pg, arpAllowPolicySuffix, nbdb.ACLDirectionFromLport, types.DefaultAllowPriority, allowMatch, nbdb.ACLActionAllow, "", policyType, nadInfo.NetNameInfo)
	}
	return
}
...
func (oc *Controller) createDefaultDenyPGAndACLs(namespace, policy string, nsInfo *namespaceInfo) error {
	...
	ingressDenyACL, ingressAllowACL := buildDenyACLs(namespace, policy, ingressPGName, aclLogging, knet.PolicyTypeIngress, oc.nadInfo)
	egressPGName := defaultDenyPortGroup(namespace, egressDefaultDenySuffix)
	egressDenyACL, egressAllowACL := buildDenyACLs(namespace, policy, egressPGName, aclLogging, knet.PolicyTypeEgress, oc.nadInfo)
	ops, err := libovsdbops.CreateOrUpdateACLsOps(oc.mc.nbClient, nil, ingressDenyACL, ingressAllowACL, egressDenyACL, egressAllowACL)
	if err != nil {
		return err
	}

	ingressPG := buildPortGroup(ingressPGName, ingressPGName, nil, []*nbdb.ACL{ingressDenyACL, ingressAllowACL}, oc.nadInfo.NetNameInfo)
	egressPG := buildPortGroup(egressPGName, egressPGName, nil, []*nbdb.ACL{egressDenyACL, egressAllowACL}, oc.nadInfo.NetNameInfo)
	ops, err = libovsdbops.CreateOrUpdatePortGroupsOps(oc.mc.nbClient, ops, ingressPG, egressPG)
	if err != nil {
		return err
	}
...
func (oc *Controller) deleteDefaultDenyPGAndACLs(namespace, policy string, nsInfo *namespaceInfo) error {
	...
	ops, err := libovsdbops.DeletePortGroupsOps(oc.mc.nbClient, nil, util.GetClusterScopedName(oc.nadInfo.Prefix+ingressPGName), util.GetClusterScopedName(oc.nadInfo.Prefix+egressPGName))
	if err != nil {
		return err
	}
	...
	ops, err = libovsdbops.DeleteACLsOps(oc.mc.nbClient, ops, nil, nil, aclsToBeDeleted...)
	if err != nil {
		return err
	}
	_, err = libovsdbops.TransactAndCheck(oc.mc.nbClient, ops)
...
func (oc *Controller) updateACLLoggingForPolicy(np *networkPolicy, logLevel string) error {
	...
	ops, err := libovsdbops.UpdateACLsLoggingOps(oc.mc.nbClient, nil, acls...)
	if err != nil {
		return err
	}
	_, err = libovsdbops.TransactAndCheck(oc.mc.nbClient, ops)
...
func (oc *Controller) setNetworkPolicyACLLoggingForNamespace(ns string, nsInfo *namespaceInfo) error {
	...
	for _, policyType := range []knet.PolicyType{knet.PolicyTypeIngress, knet.PolicyTypeEgress} {
		denyACL, _ := buildDenyACLs(ns, "", targetPortGroupName(nsInfo.portGroupIngressDenyName, nsInfo.portGroupEgressDenyName, policyType), nsInfo.aclLogging.Deny, policyType, oc.nadInfo)
		var err error
		ovsDBOps, err = libovsdbops.UpdateACLsLoggingOps(oc.mc.nbClient, ovsDBOps, denyACL)
		if err != nil {
			return err
		}
	}

	if _, err := libovsdbops.TransactAndCheck(oc.mc.nbClient, ovsDBOps); err != nil {
...
func (oc *Controller) createMulticastAllowPolicy(ns string, nsInfo *namespaceInfo) error {
	...
	egressMatch := getACLMatch(portGroupName, getMulticastACLEgrMatch(), knet.PolicyTypeEgress, oc.nadInfo.NetNameInfo)
	egressACL := buildACL(ns, portGroupName, "MulticastAllowEgress", nbdb.ACLDirectionFromLport, types.DefaultMcastAllowPriority, egressMatch, nbdb.ACLActionAllow, "", knet.PolicyTypeEgress, oc.nadInfo.NetNameInfo)
	ingressMatch := getACLMatch(portGroupName, getMulticastACLIgrMatch(nsInfo), knet.PolicyTypeIngress, oc.nadInfo.NetNameInfo)
	ingressACL := buildACL(ns, portGroupName, "MulticastAllowIngress", nbdb.ACLDirectionToLport, types.DefaultMcastAllowPriority, ingressMatch, nbdb.ACLActionAllow, "", knet.PolicyTypeIngress, oc.nadInfo.NetNameInfo)
	acls := []*nbdb.ACL{egressACL, ingressACL}
	ops, err := libovsdbops.CreateOrUpdateACLsOps(oc.mc.nbClient, nil, acls...)
	if err != nil {
		return err
	}
	...
	pg := buildPortGroup(portGroupName, ns, ports, acls, oc.nadInfo.NetNameInfo)
...
func deleteMulticastAllowPolicy(nbClient libovsdbclient.Client, ns string, nadInfo *util.NetAttachDefInfo) error {
	portGroupName := hashedPortGroup(ns)
	// ACLs referenced by the port group wil be deleted by db if there are no other references
	err := libovsdbops.DeletePortGroups(nbClient, util.GetClusterScopedName(nadInfo.Prefix+portGroupName))
	if err != nil {
		return fmt.Errorf("failed deleting port group %s: %v", util.GetClusterScopedName(nadInfo.Prefix+portGroupName), err)
	}
...
func (oc *Controller) createDefaultDenyMulticastPolicy() error {
	...
	egressACL := buildACL("", types.ClusterPortGroupName, "DefaultDenyMulticastEgress", nbdb.ACLDirectionFromLport, types.DefaultMcastDenyPriority, match, nbdb.ACLActionDrop, "", knet.PolicyTypeEgress, oc.nadInfo.NetNameInfo)

	// By default deny any ingress multicast traffic to any pod.
	ingressACL := buildACL("", types.ClusterPortGroupName, "DefaultDenyMulticastIngress", nbdb.ACLDirectionToLport, types.DefaultMcastDenyPriority, match, nbdb.ACLActionDrop, "", knet.PolicyTypeIngress, oc.nadInfo.NetNameInfo)

	ops, err := libovsdbops.CreateOrUpdateACLsOps(oc.mc.nbClient, nil, egressACL, ingressACL)
	if err != nil {
		return err
	}

	ops, err = libovsdbops.AddACLsToPortGroupOps(oc.mc.nbClient, ops, util.GetClusterScopedName(oc.nadInfo.Prefix+types.ClusterPortGroupName), egressACL, ingressACL)
	if err != nil {
		return err
	}
	...
	ops, err = libovsdbops.DeletePortGroupsOps(oc.mc.nbClient, ops, util.GetClusterScopedName(oc.nadInfo.Prefix+legacyMulticastDefaultDenyPortGroup))
	if err != nil {
		return err
	}
...
func (oc *Controller) createDefaultAllowMulticastPolicy() error {
	...
	egressMatch := getACLMatch(types.ClusterRtrPortGroupName, mcastMatch, knet.PolicyTypeEgress, oc.nadInfo.NetNameInfo)
	egressACL := buildACL("", types.ClusterRtrPortGroupName, "DefaultAllowMulticastEgress", nbdb.ACLDirectionFromLport, types.DefaultMcastAllowPriority, egressMatch, nbdb.ACLActionAllow, "", knet.PolicyTypeEgress, oc.nadInfo.NetNameInfo)

	ingressMatch := getACLMatch(types.ClusterRtrPortGroupName, mcastMatch, knet.PolicyTypeIngress, oc.nadInfo.NetNameInfo)
	ingressACL := buildACL("", types.ClusterRtrPortGroupName, "DefaultAllowMulticastIngress", nbdb.ACLDirectionToLport, types.DefaultMcastAllowPriority, ingressMatch, nbdb.ACLActionAllow, "", knet.PolicyTypeIngress, oc.nadInfo.NetNameInfo)
	...
	ops, err = libovsdbops.AddACLsToPortGroupOps(oc.mc.nbClient, ops, util.GetClusterScopedName(oc.nadInfo.Prefix+types.ClusterRtrPortGroupName), egressACL, ingressACL)
	if err != nil {
		return err
	}
...
// lock.
func podAddAllowMulticastPolicy(nbClient libovsdbclient.Client, ns string, portInfo *lpInfo, netNameInfo util.NetNameInfo) error {
	portGroupName := util.GetClusterScopedName(netNameInfo.Prefix + hashedPortGroup(ns))
	return libovsdbops.AddPortsToPortGroup(nbClient, portGroupName, portInfo.uuid)
}
...
func podDeleteAllowMulticastPolicy(nbClient libovsdbclient.Client, ns string, portUUID string, netNameInfo util.NetNameInfo) error {
	portGroupName := util.GetClusterScopedName(netNameInfo.Prefix + hashedPortGroup(ns))
	return libovsdbops.DeletePortsFromPortGroup(nbClient, portGroupName, portUUID)
}
...
// portGroupIngressDenyName and portGroupEgressDenyName is the portGroupName without network prefix
func (oc *Controller) handleLocalPodSelectorAddFunc(policy *knet.NetworkPolicy, np *networkPolicy,
	...
	ops, err = libovsdbops.AddPortsToPortGroupOps(oc.mc.nbClient, ops, util.GetClusterScopedName(oc.nadInfo.Prefix+portGroupEgressDenyName), egressDenyPorts...)
	if err != nil {
		oc.processLocalPodSelectorDelPods(np, obj)
		errs = append(errs, err)
	}

	ops, err = libovsdbops.AddPortsToPortGroupOps(oc.mc.nbClient, ops, util.GetClusterScopedName(oc.nadInfo.Prefix+np.portGroupName), policyPorts...)
	if err != nil {
		oc.processLocalPodSelectorDelPods(np, obj)
		errs = append(errs, err)
	}
...
func (oc *Controller) createNetworkPolicy(np *networkPolicy, policy *knet.NetworkPolicy, aclLogDeny, aclLogAllow,
	portGroupIngressDenyName, portGroupEgressDenyName string, statelessACL bool) error {
...
func (oc *Controller) addNetworkPolicy(policy *knet.NetworkPolicy) error {
	...
	if err := oc.createNetworkPolicy(np, policy, aclLogDeny, aclLogAllow,
		portGroupIngressDenyName, portGroupEgressDenyName, statelessACL); err != nil {
...
func (oc *Controller) destroyNetworkPolicy(np *networkPolicy, lastPolicy bool) error {
	...
	if lastPolicy {
		ops, err = libovsdbops.DeletePortGroupsOps(oc.mc.nbClient, ops, util.GetClusterScopedName(oc.nadInfo.Prefix+ingressPGName))
		if err != nil {
			return fmt.Errorf("failed to make ops for delete ingress port group: %s, for policy: %s/%s, error: %v",
				util.GetClusterScopedName(oc.nadInfo.Prefix+ingressPGName), np.namespace, np.name, err)
		}
		ops, err = libovsdbops.DeletePortGroupsOps(oc.mc.nbClient, ops, util.GetClusterScopedName(oc.nadInfo.Prefix+egressPGName))
		if err != nil {
			return fmt.Errorf("failed to make ops for delete egress port group: %s, for policy: %s/%s, error: %v",
				util.GetClusterScopedName(oc.nadInfo.Prefix+egressPGName), np.namespace, np.name, err)
		}
	} else {
		ops, err = libovsdbops.DeletePortsFromPortGroupOps(oc.mc.nbClient, ops, util.GetClusterScopedName(oc.nadInfo.Prefix+ingressPGName), ingressDenyPorts...)
		if err != nil {
			return fmt.Errorf("failed to make ops for ingress port group: %s, for policy: %s/%s, to remove ports: %#v,"+
				" error: %v", util.GetClusterScopedName(oc.nadInfo.Prefix+ingressPGName), np.namespace, np.name, ingressDenyPorts, err)
		}
		ops, err = libovsdbops.DeletePortsFromPortGroupOps(oc.mc.nbClient, ops, util.GetClusterScopedName(oc.nadInfo.Prefix+egressPGName), egressDenyPorts...)
		if err != nil {
			return fmt.Errorf("failed to make ops for egress port group: %s, for policy: %s/%s, to remove ports: %#v,"+
				" error: %v", util.GetClusterScopedName(oc.nadInfo.Prefix+egressPGName), np.namespace, np.name, egressDenyPorts, err)
		}
	}

	// Delete the port group
	ops, err = libovsdbops.DeletePortGroupsOps(oc.mc.nbClient, ops, util.GetClusterScopedName(oc.nadInfo.Prefix+np.portGroupName))
	if err != nil {
...
func (oc *Controller) handlePeerPodSelectorAddUpdate(gp *gressPolicy, objs ...interface{}) error {
	...
	if err := gp.addPeerPods(oc, pods...); err != nil && !errors.Is(err, util.ErrNoPodIPFound) {
		return err
	}
	return nil
}
...
func (oc *Controller) handlePeerServiceAdd(gp *gressPolicy, service *kapi.Service) error {
	...
	return gp.addPeerSvcVip(oc.mc.nbClient, service)
}
...
func (oc *Controller) handlePeerServiceDelete(gp *gressPolicy, service *kapi.Service) error {
	return gp.deletePeerSvcVip(oc.mc.nbClient, service)
}
...
func (oc *Controller) handlePeerNamespaceSelectorOnUpdate(np *networkPolicy, gp *gressPolicy, doUpdate func() bool) error {
	...
	if !np.deleted && doUpdate() {
		...
		ops, err = libovsdbops.AddACLsToPortGroupOps(oc.mc.nbClient, ops, util.GetClusterScopedName(oc.nadInfo.Prefix+np.portGroupName), acls...)
		if err != nil {
			return err
		}
		_, err = libovsdbops.TransactAndCheck(oc.mc.nbClient, ops)
