...
func (oc *Controller) handleLocalPodSelector(
	...
	retryLocalPods := NewRetryObjs(factory.LocalPodSelectorTypeVar)
	retryLocalPods.namespaceForFilteredHandler = policy.Namespace
	retryLocalPods.labelSelectorForFilteredHandler = sel
	retryLocalPods.syncFunc = handleInitialItems
	retryLocalPods.extraParameters = &NetworkPolicyExtraParameters{
		policy:                   policy,
		np:                       np,
...
func (oc *Controller) destroyNetworkPolicy(np *networkPolicy, lastPolicy bool) error {
	klog.Infof("destroyNetworkPolicy started on %s/%s", np.name, np.namespace)
	np.Lock()
	defer np.Unlock()
	np.deleted = true
...
func (oc *Controller) handlePeerService(
	...
	retryPeerServices := NewRetryObjs(factory.PeerServiceTypeVar)
	retryPeerServices.namespaceForFilteredHandler = policy.Namespace
	retryPeerServices.extraParameters = &NetworkPolicyExtraParameters{gp: gp}
...
func (oc *Controller) handlePeerPodSelector(
	...
	// label selector
	retryPeerPods := NewRetryObjs(factory.PeerPodSelectorTypeVar)
	retryPeerPods.namespaceForFilteredHandler = policy.Namespace
	retryPeerPods.labelSelectorForFilteredHandler = sel
	retryPeerPods.extraParameters = &NetworkPolicyExtraParameters{gp: gp}
...
func (oc *Controller) handlePeerNamespaceAndPodSelector(
	...
	retryPeerNamespaces := NewRetryObjs(factory.PeerNamespaceAndPodSelectorTypeVar)
	retryPeerNamespaces.labelSelectorForFilteredHandler = nsSel
	retryPeerNamespaces.extraParameters = &NetworkPolicyExtraParameters{
		gp:          gp,
		np:          np,
...
func (oc *Controller) handlePeerNamespaceSelector(
	...
	retryPeerNamespaces := NewRetryObjs(factory.PeerNamespaceSelectorTypeVar)
	retryPeerNamespaces.labelSelectorForFilteredHandler = sel
	retryPeerNamespaces.extraParameters = &NetworkPolicyExtraParameters{gp: gress, np: np}
