...
func (oc *Layer3UserDefinedNetworkController) ReconcileNode(oldNode, newNode *corev1.Node, oldState, newState *nodecontroller.NodeAnnotationState) error {
	...
	if oc.isLocalZoneNode(newNode) {
		...
		if oldNode == nil {
			...
			clusterRtrSync := failed || nodeChassisChanged(oldNode, newNode) || nodeSubnetChange || oc.skipPinnedLSChanged(oldNode, newNode)
...
func (oc *Layer3UserDefinedNetworkController) GetNetworkInterConnectInfo() *networkmanager.NetworkInterConnectInfo {
