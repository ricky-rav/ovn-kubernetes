...
func (c *PortCache) get(podNamespace, podName, nadName string) (*lpInfo, error) {
	...
	if nadName == types.DefaultNetworkName {
		...
		logicalPort = util.GetUserDefinedNetworkLogicalPortName(podNamespace, podName, nadName)
	}
	c.RLock()
	defer c.RUnlock()
