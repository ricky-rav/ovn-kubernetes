...
func (c *PortCache) get(podNamespace, podName, nadKey string) (*lpInfo, error) {
	var logicalPort string

	podDesc := fmt.Sprintf("%s/%s", podNamespace, podName)
	if nadKey == types.DefaultNetworkName {
		logicalPort = util.GetLogicalPortName(podNamespace, podName)
	} else {
		logicalPort = util.GetUserDefinedNetworkLogicalPortName(podNamespace, podName, nadKey)
	}
	c.RLock()
	defer c.RUnlock()
	if infoMap, ok := c.cache[podDesc]; ok {
