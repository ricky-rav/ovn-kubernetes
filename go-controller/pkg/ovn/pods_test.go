...
func newPodWithLabels(namespace, name, node, podIP string, additionalLabels map[string]string, hostIP ...string) *v1.Pod {
	podIPs := []v1.PodIP{}
	if podIP != "" {
...
func (p testPod) getAnnotationsJson() string {
	...
	"gateway_ips": ["` + p.nodeGWIP + `"], "mtu":"` + p.podMTU + `", "ip_address":"` + p.podIP + `/24", "gateway_ip": "` + p.nodeGWIP + `"}}`
}
...
func getExpectedDataPodsAndSwitches(pods []testPod, nodes []string) []libovsdbtest.TestData {
	...
	for _, pod := range pods {
		portName := util.GetLogicalPortName(pod.namespace, pod.podName, "", true)
		var lspUUID string
	...
	ginkgo.Context("during execution", func() {
		...
		ginkgo.It("reconciles an existing pod", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				defer cancel()
				lsl := []nbdb.LogicalSwitch{}
				err := fakeOvn.controller.mc.nbClient.WhereCache(
					func(ls *nbdb.LogicalSwitch) bool {
		...
		ginkgo.It("allows allocation after pods are completed", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				logicalPort := util.GetLogicalPortName(myPod.Namespace, myPod.Name, "", true)
				gomega.Eventually(func() bool {
		...
		//		fakeOvn.controller.WatchPods()
		//		// sleep long enough for TransactWithRetry to fail, causing pod add to fail
		//		time.Sleep(2*ovstypes.OVSDBTimeout + time.Second)
		//
		...
		ginkgo.It("reconciles a deleted pod", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				t.populateLogicalSwitchCache(fakeOvn, getLogicalSwitchUUID(fakeOvn.controller.mc.nbClient, "node1"))
	...
	ginkgo.Context("on startup", func() {
		...
		ginkgo.It("reconciles an existing pod with an existing logical switch port", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				t1.populateLogicalSwitchCache(fakeOvn, getLogicalSwitchUUID(fakeOvn.controller.mc.nbClient, "node1"))
				t2.populateLogicalSwitchCache(fakeOvn, getLogicalSwitchUUID(fakeOvn.controller.mc.nbClient, "node2"))
		...
		ginkgo.It("Negative test: fails to add existing pod with an existing logical switch port on wrong node", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				gomega.Eventually(func() bool {
					return fakeOvn.controller.retryPods.checkRetryObj(myPod1Key)
				}).Should(gomega.BeTrue())
		...
		ginkgo.It("reconciles a terminating pod with no node", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				logicalPort := util.GetLogicalPortName(t.namespace, t.podName, "", true)
