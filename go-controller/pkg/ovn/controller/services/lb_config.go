...
func buildServiceLBConfigs(service *corev1.Service, endpointSlices []*discovery.EndpointSlice, nodeInfos []nodeInfo,
	...
	for _, svcPort := range service.Spec.Ports {
		...
		if hasHostEndpoints(ips, netInfo) || internalTrafficLocal || preferLocal {
			...
			for _, cfg := range configs {
				...
				for _, entry := range cfg.clusterEndpoints {
					...
					if needsPreferLocal(service) && cfg.localSelectedIPs.Len() > 0 {
						// Build fallback targets (cluster IPs minus prefer-local IPs).
						...
						selectV4, selectV6 := fallbackV4, fallbackV6
						if localEPs, ok := cfg.nodeEndpoints[node.name]; ok {
							localEntry := localEPs.GetEntryByPort(entry.Port)
							var localSelV4, localSelV6 []string
							for _, ip := range localEntry.V4IPs {
								if cfg.localSelectedIPs.Has(ip) {
									localSelV4 = append(localSelV4, ip)
								}
							}
							for _, ip := range localEntry.V6IPs {
								if cfg.localSelectedIPs.Has(ip) {
									localSelV6 = append(localSelV6, ip)
								}
							}
							if len(localSelV4) > 0 {
								selectV4 = localSelV4
						...
						switchV4targets = append(switchV4targets, joinHostsPort(selectV4, entry.Port)...)
						switchV6targets = append(switchV6targets, joinHostsPort(selectV6, entry.Port)...)

						// Router: always fallback only.
						routerV4targets = append(routerV4targets, joinHostsPort(fallbackV4, entry.Port)...)
						routerV6targets = append(routerV6targets, joinHostsPort(fallbackV6, entry.Port)...)
					} else {
