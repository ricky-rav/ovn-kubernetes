...
var _ = ginkgo.Describe("Zone Interconnect Operations", func() {
	...
	ginkgo.Context("Default network", func() {
		ginkgo.BeforeEach(func() {
			...
			testNodesRouteInfo = map[string]map[string]string{
				"node1": {"node-subnets": "10.244.2.0/24", "ts-ip": "168.254.0.2", "host-route": "100.64.0.2/32"},
				"node2": {"node-subnets": "10.244.3.0/24", "ts-ip": "168.254.0.3", "host-route": "100.64.0.3/32"},
				"node3": {"node-subnets": "10.244.4.0/24", "ts-ip": "168.254.0.4", "host-route": "100.64.0.4/32"},
			}
			initialNBDB = []libovsdbtest.TestData{
	...
	ginkgo.Context("Secondary networks", func() {
		...
		ginkgo.It("Basic checks", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				netInfo, err := util.NewNetInfo(&ovncnitypes.NetConf{NetConf: cnitypes.NetConf{Name: "blue"}, Topology: types.Layer3Topology}, nil)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	ginkgo.Context("Error scenarios", func() {
		ginkgo.It("Missing annotations and error scenarios for local node", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				testNode4.Annotations[ovnNodeGRLRPAddrAnnotaton] = "{\"ipv4\":\"100.64.0.5/16\"}"
				err = zoneICHandler.AddLocalZoneNode(&testNode4)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())

				testNodesRouteInfo = map[string]map[string]string{
					"node4": {"node-subnets": "10.244.5.0/24", "ts-ip": "168.254.0.5", "host-route": "100.64.0.5/32"},
				}
