...
var _ = ginkgo.Describe("OVN Egress Gateway Operations", func() {
	...
	ginkgo.BeforeEach(func() {
		...
		if nodeSelector, err := metav1.ParseToLabelSelector("k8s.ovn.org/ovn-managed=false"); err == nil {
			if selector, err := metav1.LabelSelectorAsSelector(nodeSelector); err == nil {
				config.Kubernetes.NoHostSubnetNodes = selector
			}
		}
	...
	ginkgo.Context("SNAT on gateway router operations", func() {
		ginkgo.It("add/delete SNAT per pod on gateway router", func() {
			app.Action = func(*cli.Context) error {
				...
				finalNB = append(finalNB, getDefaultNetExpectedPodsAndSwitches([]testPod{t}, []string{nodeName})...)
				t.populateLogicalSwitchCache(fakeOvn)
				injectNode(fakeOvn)
				err := fakeOvn.controller.WatchNamespaces()
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
