...
func (oc *DefaultNetworkController) ReconcileNode(oldNode, newNode *corev1.Node, oldState, newState *nodecontroller.NodeAnnotationState) error {
	...
	if oc.isLocalZoneNode(newNode) {
		...
		if oldNode == nil {
			...
			clusterRtrSync := failed || nodeChassisChanged(oldNode, newNode) || defaultNodeSubnetChangedWithState(oldNode, newNode, oldState, newState) || oc.skipPinnedLSChanged(oldNode, newNode)
