...
func (oc *Controller) recordErrorEvent(objType reflect.Type, obj interface{}, reason string, err error) {
	switch objType {
	case factory.PodType:
		pod := obj.(*kapi.Pod)
		oc.recordPodEvent(reason, err, pod)
	}
}
...
func (oc *Controller) updateResource(objectsToRetry *retryObjs, oldObj, newObj interface{}, inRetryCache bool) error {
	...
	case factory.NodeType:
		...
		clusterRtrSync := failed || chassisChanged || subnetChanged || skipPinnedLSChanged(oldNode, newNode, oc.nadInfo)
		_, failed = oc.mgmtPortFailed.Load(newNode.Name)
		mgmtSync := failed || macAddressChanged(oldNode, newNode) || subnetChanged
...
func (oc *Controller) getSyncResourcesFunc(r *retryObjs) (func([]interface{}) error, error) {
	...
	case factory.MultinetworkpolicyType:
		syncFunc = oc.syncMultiNetworkPolicies
...
func (oc *Controller) processObjectInTerminalState(objectsToRetry *retryObjs, obj interface{}, key string, event resourceEvent) {
	...
	if err := oc.deleteResource(objectsToRetry, obj, internalCacheEntry); err != nil {
		...
		oc.recordErrorEvent(objectsToRetry.oType, obj, "ErrorDeletingResource", err)
...
func (oc *Controller) addResourceHandler(obj interface{}, objectsToRetry *retryObjs) {
	...
	klog.V(5).Infof("Add event received for resource %v, key=%s, network=%s", objectsToRetry.oType, key, oc.nadInfo.NetName)
	...
	if err := oc.addResource(objectsToRetry, obj, false); err != nil {
		klog.Errorf("Failed to create %v object %s, error: %v", objectsToRetry.oType, key, err)
		oc.recordErrorEvent(objectsToRetry.oType, obj, "ErrorAddingResource", err)
		objectsToRetry.unSkipRetryObj(key) // let iterateRetryResources process this entry
		objectsToRetry.increaseFailedAttemptsCounter(key)
		return
	}
...
func (oc *Controller) updateResourceHandler(old, newer interface{}, objectsToRetry *retryObjs) {
	...
	if retryEntry != nil && retryEntry.oldObj != nil {
		...
		if err := oc.deleteResource(objectsToRetry, retryEntry.oldObj,
			...
			oc.recordErrorEvent(objectsToRetry.oType, retryEntry.oldObj, "ErrorDeletingResource", err)
		...
		if err := oc.deleteResource(objectsToRetry, old, existingCacheEntry); err != nil {
			...
			oc.recordErrorEvent(objectsToRetry.oType, old, "ErrorDeletingResource", err)
	...
	if hasUpdateFunc {
		...
		if err := oc.updateResource(objectsToRetry, old, latest, objectsToRetry.checkRetryObj(newKey)); err != nil {
		...
		if err := oc.addResource(objectsToRetry, latest, false); err != nil {
			oc.recordErrorEvent(objectsToRetry.oType, latest, "ErrorAddingResource", err)
			objectsToRetry.initRetryObjWithAdd(latest, newKey)
			objectsToRetry.unSkipRetryObj(newKey)
			objectsToRetry.increaseFailedAttemptsCounter(newKey)
...
func (oc *Controller) deleteResourceHandler(obj interface{}, objectsToRetry *retryObjs) {
