...
const retryObjInterval = 10 * time.Second
...
// checkAndSkipRetryObj it returns true if an entry with the given key exists and sets it
// to be ignored for subsequent retries; it returns false if the entry does not exist
...
func (oc *Controller) isResourceInTerminalState(objType reflect.Type, obj interface{}) bool {
	switch objType {
	case factory.PodType,
		factory.PeerPodSelectorTypeVar,
		factory.PeerPodForNamespaceAndPodSelectorType,
...
func (oc *Controller) recordGenericEvent(objType reflect.Type, obj interface{}, err error) {
	switch objType {
	case factory.PodType:
		klog.V(5).Infof("recording generic event on pod")
		pod := obj.(*kapi.Pod)
		oc.metricsRecorder.AddPod(pod.UID)
...
func (oc *Controller) addResourceObject(objectsToRetry *retryObjs, obj interface{}, fromRetryLoop bool) error {
	var err error

	switch objectsToRetry.oType {
	case factory.PodType:
		klog.Infof("addResourceObject called on pod")
		pod, ok := obj.(*kapi.Pod)
		if !ok {
	...
	case factory.PeerServiceTypeVar:
		...
		extraParameters, ok := objectsToRetry.extraParameters.(*NetworkPolicyExtraParameters)
		if !ok {
	...
	case factory.PeerNamespaceAndPodSelectorTypeVar:
		...
		retryPeerPods := NewRetryObjs(factory.PeerPodForNamespaceAndPodSelectorType)
		retryPeerPods.namespaceForFilteredHandler = namespace.Name
		retryPeerPods.labelSelectorForFilteredHandler = extraParameters.podSelector
		retryPeerPods.extraParameters = &NetworkPolicyExtraParameters{gp: extraParameters.gp}
...
func (oc *Controller) updateResourceObject(objectsToRetry *retryObjs, oldObj, newObj interface{}, isNewObjInObjectsToRetry bool) error {
	switch objectsToRetry.oType {
	case factory.PodType:
		oldPod := oldObj.(*kapi.Pod)
		newPod := newObj.(*kapi.Pod)
		return oc.ensurePod(oldPod, newPod, isNewObjInObjectsToRetry)
	...
	case factory.PeerPodSelectorTypeVar:
		...
		extraParameters, ok := objectsToRetry.extraParameters.(*NetworkPolicyExtraParameters)
		if !ok {
			return fmt.Errorf("could not cast objectsToRetry.extraParameters to " +
				" *NetworkPolicyExtraParameters")
		}
	...
	case factory.PeerPodForNamespaceAndPodSelectorType:
		...
		// consider the old object. Is this done on purpose?
		oldPod := oldObj.(*kapi.Pod)
		extraParameters, ok := objectsToRetry.extraParameters.(*NetworkPolicyExtraParameters)
		if !ok {
			return fmt.Errorf("could not cast objectsToRetry.extraParameters to " +
...
func (oc *Controller) deleteResourceObject(objectsToRetry *retryObjs, kObj, cachedObj interface{}) error {
	klog.Infof("deleteResourceObject called on objectType %v", objectsToRetry.oType)
...
func (oc *Controller) iterateRetryResourceObjects(r *retryObjs, updateAll bool) {
	...
	for objKey, entry := range r.entries {
		...
		if entry.oldObj != nil {
			...
			if err := oc.deleteResourceObject(r, entry.oldObj, entry.config); err != nil {
		...
		if entry.newObj != nil {
			klog.Infof("%v retry: Creating object for %s",
				r.oType, objKey)
			if err := oc.addResourceObject(r, entry.newObj, true); err != nil {
