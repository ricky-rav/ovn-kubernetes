...
func (oc *DefaultNetworkController) getPortInfo(pod *corev1.Pod) *lpInfo {
	...
	if util.PodWantsHostNetwork(pod) {
		...
		portInfo, _ = oc.logicalPortCache.get(pod.Namespace, pod.Name, ovntypes.DefaultNetworkName)
	}
	return portInfo
}
...
func podNodeNameLabelChanged(pod *corev1.Pod, nodeNameLabel map[string]string) bool {
...
func (oc *DefaultNetworkController) ensureLocalZonePod(oldPod, pod *corev1.Pod, addPort bool) error {
	...
	if !util.PodWantsHostNetwork(pod) && addPort {
		if err := oc.addLogicalPort(pod); err != nil {
			failure := fmt.Errorf("addLogicalPort failed for %s/%s network %s: %w", pod.Namespace, pod.Name, oc.GetNetworkName(), err)
