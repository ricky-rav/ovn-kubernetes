...
func (bnc *BaseNetworkController) syncNodeClusterRouterPort(node *corev1.Node, hostSubnets []*net.IPNet) error {
	...
	for _, skipNAD := range allSkipNADs {
		if bnc.networkManager.GetNetworkNameForNADKey(skipNAD) == bnc.GetNetworkName() {
			skipPinnedLS = true
			break
		}
	}
...
func (bnc *BaseNetworkController) WatchAdminPolicyBasedRoutes() (err error) {
	...
	filterAdminPBR := func(obj interface{}) bool {
		...
		return bnc.networkManager.GetNetworkNameForNADKey(apbr.Spec.NetworkAttachmentName) == bnc.GetNetworkName()
	}
...
func (bnc *BaseNetworkController) shouldSkipPinnedLS(node *corev1.Node) bool {
	...
	for _, nadName := range nadNames {
		...
		if !bnc.IsUserDefinedNetwork() {
			...
			if bnc.networkManager.GetNetworkNameForNADKey(nadName) == bnc.GetNetworkName() {
				return true
			}
		}
	}
...
func (bnc *BaseNetworkController) skipPinnedLSChanged(oldNode, node *corev1.Node) bool {
