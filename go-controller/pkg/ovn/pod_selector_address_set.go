...
func (bnc *BaseNetworkController) EnsurePodSelectorAddressSet(policyType knet.PolicyType, podSelector, namespaceSelector *metav1.LabelSelector,
	...
	addrSetKey = getPodSelectorKey(podSelector, namespaceSelector, namespace, string(policyType))
...
func (handlerInfo *PodSelectorAddrSetHandlerInfo) addPods(pods ...*v1.Pod) error {
	...
	for _, pod := range pods {
		if pod.Spec.HostNetwork {
			...
			if err != nil {
				return fmt.Errorf("failed while adding Hostnetwrok IP for pod %s/%s on node (%s) to address_set: %v",
...
func (handlerInfo *PodSelectorAddrSetHandlerInfo) deletePod(pod *v1.Pod) error {
	if pod.Spec.HostNetwork {
		...
		handlerInfo.nodeHostNetPodsCacheLock.Lock()
		defer handlerInfo.nodeHostNetPodsCacheLock.Unlock()
		return delHostnetworkPodIPFromAddressSet(pod.Spec.NodeName, fmt.Sprintf("%s/%s", pod.Namespace, pod.Name),
			handlerInfo.addressSet, handlerInfo.nodeHostNetPodsCache)
	}
...
func getPodSelectorKey(podSelector, namespaceSelector *metav1.LabelSelector, namespace, policyType string) string {
	...
	return namespaceKey + "_" + shortLabelSelectorString(podSelector) + "_" + policyType
}
