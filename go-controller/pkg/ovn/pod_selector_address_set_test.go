...
var _ = ginkgo.Describe("OVN PodSelectorAddressSet", func() {
	...
	ginkgo.It("creates one address set for multiple users with the same selector", func() {
		...
		peerASKey1 := getPodSelectorKey(peer.PodSelector, peer.NamespaceSelector, namespace1.Name, string(knet.PolicyTypeIngress))
		peerASIDs1 := getPodSelectorAddrSetDbIDs(peerASKey1, DefaultNetworkControllerName)
		fakeOvn.asf.EventuallyExpectEmptyAddressSetExist(peerASIDs1)
		peerASKey2 := getPodSelectorKey(peer.PodSelector, peer.NamespaceSelector, namespace1.Name, string(knet.PolicyTypeEgress))
		peerASIDs2 := getPodSelectorAddrSetDbIDs(peerASKey2, DefaultNetworkControllerName)
		fakeOvn.asf.EventuallyExpectEmptyAddressSetExist(peerASIDs2)
	...
	ginkgo.It("reconciles a completed and deleted pod whose IP has been assigned to a running pod", func() {
		...
		_, _, _, err := fakeOvn.controller.EnsurePodSelectorAddressSet(knet.PolicyTypeIngress,
