...
func newGWManager(
	controllerName, nodeName, clusterRouterName, extSwitchName, joinSwitchName string,
	coopUUID string,
	kube kube.InterfaceOVN,
	nbClient libovsdbclient.Client,
	netInfo util.NetInfo,
	...
	if !config.Gateway.DisableSNATMultipleGWs || gw.netInfo.IsPrimaryNetwork() {
		...
		if gw.cleanupDuplicateNoOverlayClusterSNATsFn != nil {
			if err := gw.cleanupDuplicateNoOverlayClusterSNATsFn(gw, gwRouter, nats); err != nil {
				return fmt.Errorf("failed to cleanup duplicate no-overlay SNATs on router %s error: %v", gw.gwRouterName, err)
			}
		}
	} else {
...
func (gw *GatewayManager) gatewayInit(
	...
	if gw.netInfo.IsDefault() {
		...
		for _, rule := range gwConfig.annoConfig.GWSNATRules {
			if err := gw.applyGWSnatRule(rule, gwRouter, gwConfig.hostSubnets, snatMap); err != nil {
				return err
			}
		}
		if err := gw.cleanStaleGwSnatRule(gwRouter, snatMap); err != nil {
			return err
		}
	}
...
func (gw *GatewayManager) addExternalSwitch(prefix, interfaceID, gatewayRouter, macAddress, physNetworkName string, ipAddresses []*net.IPNet, vlanID *uint) error {
	...
	lsw, err := libovsdbops.GetLogicalSwitch(gw.nbClient, &nbdb.LogicalSwitch{Name: externalSwitch})
	if err != nil && !errors.Is(err, libovsdbclient.ErrNotFound) {
	...
	if err == nil {
		...
		for _, port := range lsw.Ports {
			...
			lsp, err := libovsdbops.GetLogicalSwitchPort(gw.nbClient, lsp)
			if err != nil {
		...
		if len(localNetLogicalSwitchPorts) > 0 {
			err := libovsdbops.DeleteLogicalSwitchPorts(gw.nbClient, lsw, localNetLogicalSwitchPorts...)
			if err != nil {
...
func (gw *GatewayManager) Cleanup() error {
	...
	if gw.netInfo.IsDefault() {
		...
		err = gw.cleanStaleGwSnatRule(&logicalRouter, nil)
		if err != nil {
			return fmt.Errorf("failed to delete stale custom nats for node %s: %w", gw.nodeName, err)
		}
	}
