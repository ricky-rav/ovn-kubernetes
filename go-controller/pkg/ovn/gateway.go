...
func (gw *GatewayManager) addExternalSwitch(prefix, interfaceID, gatewayRouter, macAddress, physNetworkName string, ipAddresses []*net.IPNet, vlanID *uint) error {
	...
	// The localnet port name embeds the bridge name, so a bridge change (Uplink
	// nodeConfig) yields a new port name on the same switch. ovn-controller
	// forwards through the first localnet port it bound, so a leftover old port
	// keeps egress on its patch port. Delete it in the same transaction.
	...
	if err != nil {
		return fmt.Errorf("failed to build ops removing stale localnet ports from switch %s: %w", externalSwitch, err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to build ops creating ports %s and %s on switch %s: %w",
			interfaceID, externalSwitchPortToRouter, externalSwitch, err)
	}
	if _, err = libovsdbops.TransactAndCheck(gw.nbClient, ops); err != nil {
		return fmt.Errorf("failed to replace localnet port and create ports %s and %s on switch %s: %w",
			interfaceID, externalSwitchPortToRouter, externalSwitch, err)
	}
