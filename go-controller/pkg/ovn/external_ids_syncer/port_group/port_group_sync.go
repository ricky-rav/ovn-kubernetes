...
func NewPortGroupSyncer(nbClient libovsdbclient.Client, netInfo util.NetInfo) *PortGroupSyncer {
...
func (syncer *PortGroupSyncer) getPortGroupUpdateInfo(pg *nbdb.PortGroup, pgToACLs map[string][]string) (*updatePortGroupInfo, error) {
	...
	networkExternalID := pg.ExternalIDs[types.LegacyNetworkExternalID]
	if networkExternalID == "" {
...
func (syncer *PortGroupSyncer) SyncPortGroups() error {
	...
	p := func(item *nbdb.PortGroup) bool {
		if syncer.IsSecondary() {
			if item.ExternalIDs[types.LegacyNetworkExternalID] != syncer.GetNetworkName() &&
				item.ExternalIDs[types.NetworkExternalID] != syncer.GetNetworkName() {
				return false
			}
		} else {
			if item.ExternalIDs[types.LegacyNetworkExternalID] != "" || item.ExternalIDs[types.NetworkExternalID] != "" {
				return false
			}
		}
		return item.ExternalIDs[libovsdbops.OwnerControllerKey.String()] == ""
	}
	portGroupList, err := libovsdbops.FindPortGroupsWithPredicate(syncer.nbClient, p)
	if err != nil {
