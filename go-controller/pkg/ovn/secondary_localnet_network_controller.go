...
func (oc *SecondaryLocalnetNetworkController) Start(ctx context.Context) error {
	klog.Infof("Starting secondary %s network controller of network %s", oc.TopologyType(), oc.GetNetworkName())
	start := time.Now()
	defer func() {
...
func (oc *SecondaryLocalnetNetworkController) NADToInterConnect() string {
	return ""
}

func (oc *SecondaryLocalnetNetworkController) StartInterConnect(icInfo *util.InterConnectInfo) error {
	panic("unexpected call for secondary localnet Network Controller")
}

func (oc *SecondaryLocalnetNetworkController) StopInterConnect(icInfo *util.InterConnectInfo) error {
	panic("unexpected call for secondary localnet Network Controller")
}
