	...
	ginkgo.Context("WatchEgressNodes", func() {
		...
		ginkgo.It("egressIP pod recreate with same name (stateful-sets) shouldn't use stale logicalPortCache entries", func() {
			app.Action = func(*cli.Context) error {
				...
				egressPodPortInfo, err := fakeOvn.controller.logicalPortCache.get(egressPod1.Namespace, egressPod1.Name, types.DefaultNetworkName)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
		...
		ginkgo.It("egressIP pod recreate with same name (stateful-sets) shouldn't use stale logicalPortCache entries AND stale podAssignment cache entries", func() {
			app.Action = func(*cli.Context) error {
				...
				oldEgressPodPortInfo, err := fakeOvn.controller.logicalPortCache.get(egressPod1.Namespace, egressPod1.Name, types.DefaultNetworkName)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				...
				getEgressPodIP := func() string {
					newEgressPodPortInfo, err = fakeOvn.controller.logicalPortCache.get(egressPod1.Namespace, egressPod1.Name, types.DefaultNetworkName)
					gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	ginkgo.Context("WatchEgressNodes running with WatchEgressIP", func() {
					...
					libovsdbtest.TestSetup{
						NBData: []libovsdbtest.TestData{
							&nbdb.LogicalRouterPolicy{
								...
								Priority:    types.DefaultNoRereoutePriority,
								Action:      nbdb.LogicalRouterPolicyActionAllow,
								ExternalIDs: getEgressIPLRPNoReRoutePodToPodDbIDs(IPFamilyValueV4, types.DefaultNetworkName, DefaultNetworkControllerName, 0).GetExternalIDs(),
							},
							&nbdb.LogicalRouterPolicy{
								UUID:        "remove-me-UUID",
		...
		ginkgo.It("should remove stale pod SNAT referring to wrong logical port after ovnkube-master is started", func() {
			app.Action = func(*cli.Context) error {
				...
				egressPodPortInfo, err := fakeOvn.controller.logicalPortCache.get(egressPod.Namespace, egressPod.Name, types.DefaultNetworkName)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
		...
		ginkgo.It("should update SNAT and LRP nexthops during simultaneous EIP failover and ovnkube-controller restart", func() {
			app.Action = func(*cli.Context) error {
				...
				egressPod := *newPodWithLabels(eipNamespace, podName, node1Name, podV4IP, egressPodLabel)
				egressPod2 := *newPodWithLabels(eipNamespace, podName2, node3Name, podV4IP2, egressPodLabel)
				egressNamespace := newNamespace(eipNamespace)
		...
		ginkgo.DescribeTable(
			...
			func(includeUnscheduled, includeScheduledButNoIP, includeHostNetwork, interconnect, isPodRemote bool) {
				app.Action = func(*cli.Context) error {
					...
					egressPod3 := *newPodWithLabels(eipNamespace, "egress-pod3", node1Name, podV4IP, egressPodLabel)
					egressPod4 := *newPodWithLabels(eipNamespace, "egress-pod4", node1Name, podV4IP2, egressPodLabel)
					egressNamespace := newNamespace(eipNamespace)
					annotations := map[string]string{
						"k8s.ovn.org/node-primary-ifaddr": fmt.Sprintf("{\"ipv4\": \"%s\"}", node1IPv4CIDR),
						"k8s.ovn.org/node-subnets":        fmt.Sprintf("{\"default\":\"%s\"}", v4Node1Subnet),
					...
					if includeUnscheduled {
						unScheduledPod := corev1.Pod{
							ObjectMeta: newPodMeta(eipNamespace, "egress-pod", egressPodLabel),
							Spec: corev1.PodSpec{
					...
					if includeScheduledButNoIP {
						pendingScheduledPodWithNoIP = newPodWithLabels(eipNamespace, "egress-pod1", node2Name, "", egressPodLabel)
					...
					if includeHostNetwork {
						hnPod := *newPodWithLabels(eipNamespace, "egress-pod2", node2Name, node2IPv4, egressPodLabel)
						...
						libovsdbtest.TestSetup{
							NBData: []libovsdbtest.TestData{
								...
								&nbdb.NAT{
									...
									LogicalIP:   node2IPv4,
									ExternalIP:  egressIP1,
									ExternalIDs: getEgressIPNATDbIDs(egressIPName, eipNamespace, "egress-pod2", IPFamilyValueV4, DefaultNetworkControllerName).GetExternalIDs(),
									Type:        nbdb.NATTypeSNAT,
									LogicalPort: &expectedNatLogicalPort1,
									Options: map[string]string{
										"stateless": "false",
		...
		ginkgo.It("should not deadlock when two EgressIP pods are hosted and egress through opposite nodes", func() {
			...
			app.Action = func(*cli.Context) error {
				...
				egressPod2 := *newPodWithLabels(eipNamespace2, podName2, node2Name, podV4IP2, egressPodLabel)
			...
			app.Action = func(*cli.Context) error {
				...
				expectedDatabaseState := []libovsdbtest.TestData{
					...
					&nbdb.LogicalRouterPolicy{
						...
						ExternalIDs: getEgressIPLRPNoReRoutePodToPodDbIDs(IPFamilyValueV4, types.DefaultNetworkName, fakeOvn.controller.eIPC.controllerName, 1).GetExternalIDs(),
					},
					&nbdb.LogicalRouterPolicy{
						Priority:    types.DefaultNoRereoutePriority,
						...
						ExternalIDs: getEgressIPLRPNoReRoutePodToPodDbIDs(IPFamilyValueV4, types.DefaultNetworkName, fakeOvn.controller.eIPC.controllerName, 2).GetExternalIDs(),
					},
					&nbdb.LogicalRouterPolicy{
						Priority:    types.DefaultNoRereoutePriority,
						...
						ExternalIDs: getEgressIPLRPNoReRoutePodToPodDbIDs(IPFamilyValueV4, types.DefaultNetworkName, fakeOvn.controller.eIPC.controllerName, 3).GetExternalIDs(),
					},
					&nbdb.LogicalRouterPolicy{
						Priority:    types.DefaultNoRereoutePriority,
					...
					&nbdb.LogicalRouterPolicy{
						...
						ExternalIDs: getEgressIPLRPNoReRoutePodToPodDbIDs(IPFamilyValueV6, types.DefaultNetworkName, fakeOvn.controller.eIPC.controllerName, 1).GetExternalIDs(),
					},
					&nbdb.LogicalRouterPolicy{
						Priority:    types.DefaultNoRereoutePriority,
						...
						ExternalIDs: getEgressIPLRPNoReRoutePodToPodDbIDs(IPFamilyValueV6, types.DefaultNetworkName, fakeOvn.controller.eIPC.controllerName, 2).GetExternalIDs(),
					},
					&nbdb.LogicalRouterPolicy{
						Priority:    types.DefaultNoRereoutePriority,
						...
						ExternalIDs: getEgressIPLRPNoReRoutePodToPodDbIDs(IPFamilyValueV6, types.DefaultNetworkName, fakeOvn.controller.eIPC.controllerName, 3).GetExternalIDs(),
					},
					&nbdb.LogicalRouterPolicy{
						Priority:    types.DefaultNoRereoutePriority,
						Match:       fmt.Sprintf("ip4.src == 10.128.0.0/14 && ip4.dst == %s", config.Gateway.V4JoinSubnet),
					...
					&nbdb.LogicalRouter{
						Name: types.OVNClusterRouter,
						UUID: types.OVNClusterRouter + "-UUID",
						Policies: []string{"no-reroute-ipv4-1-UUID", "no-reroute-ipv4-2-UUID", "no-reroute-ipv4-3-UUID", "no-reroute-ipv4-4-UUID", "no-reroute-service-ipv4-1-UUID",
							"no-reroute-ipv6-1-UUID", "no-reroute-ipv6-2-UUID", "no-reroute-ipv6-3-UUID", "no-reroute-ipv6-4-UUID", "no-reroute-service-ipv4-2-UUID", "no-reroute-service-ipv6-1-UUID",
	...
	ginkgo.Context("on label updates", func() {
		...
		ginkgo.It("should handle EIP reassignment correctly on pod label update", func() {
			app.Action = func(*cli.Context) error {
				...
				egressPod := newPodWithLabels(eipNamespace, podName, node1Name, podV4IP, podLabel)
				egressNamespace := newNamespace(eipNamespace)
				nodes := getIPv4Nodes([]nodeInfo{{[]string{node1IPv4CIDR}, "global", node1IPv4TranSwitchIP}})
				node1 := nodes[0]
				node1.Labels = map[string]string{
