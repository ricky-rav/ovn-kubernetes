...
func NewSBClient(stopCh <-chan struct{}, opts ...client.Option) (client.Client, error) {
	return NewSBClientWithEndpoint(config.OvnSouth.GetURL(), stopCh, opts...)
}
...
func NewSBClientWithEndpoint(endpoint string, stopCh <-chan struct{}, opts ...client.Option) (client.Client, error) {
	dbModel, err := sbdb.FullDatabaseModel()
	if err != nil {
		return nil, err
	}

	c, err := newClient(endpoint, dbModel, opts...)
	if err != nil {
		return nil, err
	}
...
// local OVN NB DB.
func NewNBClient(stopCh <-chan struct{}, opts ...client.Option) (client.Client, error) {
	return NewNBClientWithEndpoint(config.OvnNorth.GetURL(), stopCh, opts...)
}
...
func NewNBClientWithEndpoint(endpoint string, stopCh <-chan struct{}, opts ...client.Option) (client.Client, error) {
