...
func UpdateLogicalRouterSetExternalIDs(nbClient libovsdbclient.Client, router *nbdb.LogicalRouter) error {
	ops, err := UpdateLogicalRouterSetExternalIDsOps(nbClient, nil, router)
	if err != nil {
		return err
	}
...
//   - Compare AllowedExtIPs if it is set in `searched`.
//
// ExemptedExtIPs is intentionally not compared because it is mutable NAT
// configuration that reconciliation may need to update on the existing row.
...
type gatewayChassisPredicate func(*nbdb.GatewayChassis) bool

// DeleteGatewayChassisWithPredicate looks up gatewayChassises from the cache based on a given predicate,
// deletes them and removes them from the provided logical router port
func DeleteGatewayChassisWithPredicate(nbClient libovsdbclient.Client, portName string, p gatewayChassisPredicate) error {
	...
	opModels := []operationModel{
		{
			ModelPredicate: p,
			ExistingResult: &deleted,
			DoAfter:        func() { port.GatewayChassis = extractUUIDsFromModels(&deleted) },
			ErrNotFound:    false,
			BulkOp:         true,
		},
		{
			Model:            port,
			ModelPredicate:   func(item *nbdb.LogicalRouterPort) bool { return item.Name == port.Name },
			OnModelMutations: []interface{}{&port.GatewayChassis},
			ErrNotFound:      true,
			BulkOp:           false,
		},
	}
