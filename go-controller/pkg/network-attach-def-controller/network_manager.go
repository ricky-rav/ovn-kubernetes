...
type networkManager interface {
	// InitDefaultNetworkController set the default network controller
	InitDefaultNetworkController(BaseNetworkController)
...
func (nm *networkManagerImpl) Stop() {
	...
	for _, networkControllerState := range nm.getAllSecondaryNetworkStates() {
		networkControllerState.controller.Stop()
	}
}

func (nm *networkManagerImpl) InitDefaultNetworkController(defaultNetworkController BaseNetworkController) {
	if defaultNetworkController != nil {
		nm.setNetwork(defaultNetworkController.GetNetworkName(), util.InitDefaultNetInfo())
		nm.setNetworkState(defaultNetworkController.GetNetworkName(), &networkControllerState{controller: defaultNetworkController})
	}
}
...
func (nm *networkManagerImpl) DeleteNetwork(network string) {
	if network == types.DefaultNetworkName {
		klog.Fatalf("Unexpected default network to be deleted")
	}
...
func (nm *networkManagerImpl) getAllSecondaryNetworks() []util.BasicNetInfo {
	nm.RLock()
	defer nm.RUnlock()
...
func (nm *networkManagerImpl) getAllSecondaryNetworkStates() []*networkControllerState {
	nm.RLock()
	defer nm.RUnlock()
	networkStates := make([]*networkControllerState, 0, len(nm.networks))
	for _, state := range nm.networkControllers {
		if state.controller.IsDefault() {
			continue
		}
...
func (nm *networkManagerImpl) sync(network string) error {
	...
	isDefault := network == types.DefaultNetworkName
	...
	if dispose {
		if isDefault {
			klog.Fatalf("Unexpected: try to cleanup and stop default network controller")
		}
		if !have.stoppedAndDeleting {
			have.controller.(NetworkController).Stop()
		}
		have.stoppedAndDeleting = true
		err := have.controller.(NetworkController).Cleanup()
		if err != nil {
	...
	if have != nil && !dispose {
		have.controller.SetNADs(want.GetNADConfigs())
		return nil
	}

	if isDefault {
		klog.Fatalf("Unexpected: try to recreate default network controller")
	}
...
func (nm *networkManagerImpl) syncAll() error {
	...
	validNetworks := nm.getAllSecondaryNetworks()
