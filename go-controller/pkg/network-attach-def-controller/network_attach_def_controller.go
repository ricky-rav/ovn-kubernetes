...
type NADController interface {
	Start(BaseNetworkController) error
	Stop()
...
type NetAttachDefinitionController struct {
	...
	// skip managing NAD of Default network
	skipDefaultNetwork bool
}
...
func (nadController *NetAttachDefinitionController) Start(defaultNetworkController BaseNetworkController) error {
	if defaultNetworkController != nil {
		nadController.networkManager.InitDefaultNetworkController(defaultNetworkController)
		// add a NAD of invalid key to not conflict with real NAD
		err := nadController.syncNADCommon("default-namespace/default", defaultNetworkController.GetNetworkName(),
			util.InitDefaultNetInfo(), nil)
		if err != nil {
			return err
		}
	} else {
		nadController.skipDefaultNetwork = true
	}
...
func (nadController *NetAttachDefinitionController) syncNAD(key string, nad *nettypes.NetworkAttachmentDefinition) error {
	...
	var err error
	var newNADConfig *util.NADConfig

	if nad != nil {
		nadNetwork, newNADConfig, err = util.ParseNADInfo(nad)
		if err != nil {
...
func (nadController *NetAttachDefinitionController) syncNADCommon(key, nadNetworkName string, nadNetwork util.NetInfo, newNADConfig *util.NADConfig) error {
	var oldNetwork, ensureNetwork util.NetInfo
	...
	case len(currentNetwork.GetNADConfigs()) == 1 && currentNetwork.GetFirstNAD() == key:
		// the NAD is the only NAD referring to the network, an existing incompatible
	...
	case oldNetwork == nil:
		...
		fallthrough
	default:
		err = fmt.Errorf("%s: NAD %s CNI config does not match that of network %s", nadController.name, key, nadNetwork)
	}
	...
	if oldNetwork != nil {
		...
		// if ensureNetwork is the same as oldNetwork, the NAD may only has NACConfig change and will be added back later,
		// leave oldNetwork update to be done later
		if ensureNetwork == nil || !oldNetwork.Equals(ensureNetwork) {
			if len(oldNetwork.GetNADConfigs()) == 0 {
	...
	if ensureNetwork.IsDefault() && nadController.skipDefaultNetwork {
	...
	ensureNetwork.AddNADs(map[string]*util.NADConfig{key: newNADConfig})
...
func nadNeedsUpdate(oldNAD, newNAD *nettypes.NetworkAttachmentDefinition) bool {
	...
	// spec change or annotation (per-NAD Configuration change)
	return !reflect.DeepEqual(oldNAD.Spec, newNAD.Spec) || !reflect.DeepEqual(oldNAD.Annotations, newNAD.Annotations)
}
...
func (nadController *NetAttachDefinitionController) GetActiveNetworkForNamespace(namespace string) (util.NetInfo, error) {
	...
	if primaryNAD != "" {
		...
		n.SetNADs(map[string]*util.NADConfig{primaryNAD: nil})
		return n, nil
	}
