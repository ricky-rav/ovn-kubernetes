...
import (
	...
	ipresvclientset "github.com/ovn-kubernetes/ovn-kubernetes/go-controller/pkg/crd/ipreservation/v1beta1/apis/clientset/versioned"
...
func NewOVNClientset(conf *config.KubernetesConfig) (*OVNClientset, error) {
	...
	networkConnectClientset, err := networkconnectclientset.NewForConfig(kconfig)
	if err != nil {
		return nil, err
	}
	...
	return &OVNClientset{
		...
		NetworkConnectClient:      networkConnectClientset,
...
// ValidateProtocol checks if the protocol is a valid corev1.Protocol type (TCP, UDP, or SCTP) or returns an error
