...
func (h *nodeEventHandler) UpdateResource(oldObj, newObj interface{}, _ bool) error {
	...
	case factory.EndpointSliceForStaleConntrackRemovalType:
		...
		if (config.OvnKubeNode.Mode == types.NodeModeDPU && config.OvnKubeNode.IsPrimaryDPU) ||
			...
			if err := h.nc.reconcileConntrackUponEndpointSliceEvents(
				oldEndpointSlice, newEndpointSlice); err != nil {
...
func (h *nodeEventHandler) DeleteResource(obj, _ interface{}) error {
	...
	case factory.EndpointSliceForStaleConntrackRemovalType:
		...
		if (config.OvnKubeNode.Mode == types.NodeModeDPU && config.OvnKubeNode.IsPrimaryDPU) ||
			config.OvnKubeNode.Mode == types.NodeModeFull {
			if err := h.nc.reconcileConntrackUponEndpointSliceEvents(endpointslice, nil); err != nil {
				errors = append(errors, err)
			}
		}
		if config.OvnKubeNode.Mode != types.NodeModeDPU {
...
func (nc *DefaultNodeNetworkController) syncFirewallZoneForEndpointSlices(_ []interface{}) error {
...
func (h *nodeEventHandler) SyncFunc(objs []interface{}) error {
	...
	if h.syncFunc != nil {
		...
		case factory.EndpointSliceForStaleConntrackRemovalType:
			syncFunc = nil
			if config.OvnKubeNode.Mode != types.NodeModeDPU {
				syncFunc = h.nc.syncFirewallZoneForEndpointSlices
			}
