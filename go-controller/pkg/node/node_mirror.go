...
func (bnnc *BaseNodeNetworkController) syncPortMirrors(portMirrors []interface{}) error {
	...
	for _, portMirror := range portMirrors {
		...
		s, ok := bnnc.portMirrorIDToSFMap.Load(portMirrorID)
		if ok {
			sfInfo := s.(*sfDetails)
			sfInfo.mirrorCount++
		...
		bnnc.setSFPortNum(sfInfo.sfNum)
		// store the sfInfo
		bnnc.portMirrorIDToSFMap.Store(portMirrorID, sfInfo)
	}
	return nil
}
...
func (bnnc *BaseNodeNetworkController) watchPortMirrorDPU() error {
	start := time.Now()
	bnnc.portMirrorRetryQueueDPU = workqueue.NewNamedRateLimitingQueue(workqueue.DefaultControllerRateLimiter(), "portMirror")
	_, err := bnnc.watchFactory.AddPortMirrorHandler(cache.ResourceEventHandlerFuncs{
	...
	go func() {
		...
		for {
			select {
			case <-ticker.C:
				bnnc.syncPortMirrorsDPUPeriodic()
			case <-bnnc.stopChan:
				ticker.Stop()
				bnnc.portMirrorRetryQueueDPU.ShutDown()
				return
			}
		}
	}()
	...
	go func() {
		for bnnc.retryPortMirrorOperationsDPU() {
		}
	}()
...
func (bnnc *BaseNodeNetworkController) retryOnAddFailure(sfInfo *sfDetails, errMsg, key string, pm *util.PortMirror, pod *kapi.Pod) {
	...
	util.UpdatePortMirrorStatusOnError(pm, bnnc.Kube, errMsg, key)
	bnnc.requeueSinkPodAddForPortMirror(pm, pod)
}
...
func (bnnc *BaseNodeNetworkController) handlePortMirrorSinkPodAdd(pm *util.PortMirror, pod *kapi.Pod) error {
	...
	if _, ok := pm.FailedPortMirrorOps[sinkPodKeyForFailedOps]; ok {
		util.ClearPortMirrorErrorMessage(pm, bnnc.Kube, sinkPodKeyForFailedOps)
	}
	...
	s, ok := bnnc.portMirrorIDToSFMap.Load(pm.SinkLocalDetails.PortMirrorId)
	if ok {
		sfInfo = s.(*sfDetails)
		...
		updateErr := util.UpdatePortMirrorStatusWithRetry(bnnc.Kube, pm.Namespace, pm.Name, types.OvnK8sStatusSucceeded, "", "")
		if updateErr != nil {
	...
	if err != nil {
		...
		errMsg := fmt.Sprintf("%s - Failed to get %s uplink port info for %s on DPU %s",
			util.MessagePrefixPortMirrorErr, types.K8sMgmtIntfName, sinkPodKeyForFailedOps, bnnc.dpuName)
		bnnc.retryOnAddFailure(nil, errMsg, sinkPodKeyForFailedOps, pm, pod)
		return fmt.Errorf("failed to get uplink port info for %s on DPU %s for sink pod %s to create sf: (%v)",
			types.K8sMgmtIntfName, bnnc.dpuName, util.GetNamespacedName(pod.Namespace, pod.Name), err)
	}
	sfInfo, err = bnnc.getSFInfo(physUplinkPort, physUplinkPortNum, pm.SinkLocalDetails.SinkSFNum)
	if err != nil {
		// retry
		errMsg := fmt.Sprintf("%s - Failed to create SF for %s on DPU %s", util.MessagePrefixPortMirrorErr,
			sinkPodKeyForFailedOps, bnnc.dpuName)
		bnnc.retryOnAddFailure(nil, errMsg, sinkPodKeyForFailedOps, pm, pod)
		return fmt.Errorf("failed to create a sf on DPU %s for sink pod %s: (%v)",
			bnnc.dpuName, util.GetNamespacedName(pod.Namespace, pod.Name), err)
	}
	...
	if pm.SinkLocalDetails.SinkIfName != "" {
		...
		if err != nil {
			errMsg := fmt.Sprintf("%s - Failed to rename sf netdevice for %s on DPU %s",
				util.MessagePrefixPortMirrorErr, sinkPodKeyForFailedOps, bnnc.dpuName)
			bnnc.retryOnAddFailure(sfInfo, errMsg, sinkPodKeyForFailedOps, pm, pod)
	...
	if err != nil {
		errMsg := fmt.Sprintf("%s - Failed to add SfRep %s to br-int for %s on DPU %s",
			util.MessagePrefixPortMirrorErr, sfInfo.sfRepName, sinkPodKeyForFailedOps, bnnc.dpuName)
		bnnc.retryOnAddFailure(sfInfo, errMsg, sinkPodKeyForFailedOps, pm, pod)
	...
	bnnc.portMirrorIDToSFMap.Store(pm.SinkLocalDetails.PortMirrorId, sfInfo)
	// update the status to success
	updateErr := util.UpdatePortMirrorStatusWithRetry(bnnc.Kube, pm.Namespace, pm.Name, types.OvnK8sStatusSucceeded, "", "")
	if updateErr != nil {
		klog.Errorf("Failed to update portmirror %s/%s status to success state :(%v)", pm.Namespace, pm.Name, updateErr)
...
func (bnnc *BaseNodeNetworkController) handlePortMirrorSinkPodDelete(pm *util.PortMirror, pod *kapi.Pod) error {
	...
	if sfInfo.mirrorCount == 0 {
		...
		bnnc.portMirrorIDToSFMap.Delete(pm.SinkLocalDetails.PortMirrorId)
...
func (bnnc *BaseNodeNetworkController) unConfigureSFInterface(sfInfo *sfDetails) error {
...
func (bnnc *BaseNodeNetworkController) handleSinkPodSelectors(pm *util.PortMirror) {
	klog.Infof("Adding portmirror podhandler %s/%s", pm.Namespace, pm.Name)
	pm.SinkLocalDetails.PodHandler, _ = bnnc.watchFactory.AddFilteredPodHandler(pm.Namespace, pm.SinkLocalDetails.PodSelector,
...
func (bnnc *BaseNodeNetworkController) addDPUPortMirror(portMirror *portmirror.PortMirror) error {
	...
	pmList, ok := bnnc.mirrorIDToPortMirrorMap.Load(pMirror.SinkLocalDetails.PortMirrorId)
	if !ok {
		...
		if !match {
			...
			util.UpdatePortMirrorStatusOnError(pMirror, bnnc.Kube, errMsg, bnnc.dpuName)
			portMirrorIDUnlock()
	...
	bnnc.mirrorIDToPortMirrorMap.Store(pMirror.SinkLocalDetails.PortMirrorId, portMirrorList)
	portMirrorIDUnlock()

	bnnc.portMirrorMap.Store(portMirrorKey, pMirror)
	portMirrorNameUnlock()
	bnnc.handleSinkPodSelectors(pMirror)

	return nil
}

func (bnnc *BaseNodeNetworkController) deleteDPUPortMirror(portMirror *portmirror.PortMirror) error {
	portMirrorNameUnlock := util.GetLockByPMName(portMirror.Namespace, portMirror.Name)
	defer portMirrorNameUnlock()

	portMirrorKey := util.GetNamespacedName(portMirror.Namespace, portMirror.Name)
	klog.Infof("Deleting portmirror %s/%s", portMirror.Namespace, portMirror.Name)
	p, ok := bnnc.portMirrorMap.LoadAndDelete(portMirrorKey)
	if !ok {
	...
	bnnc.watchFactory.RemovePodHandler(pm.SinkLocalDetails.PodHandler.(*factory.Handler))
	...
	pList, ok := bnnc.mirrorIDToPortMirrorMap.Load(pm.SinkLocalDetails.PortMirrorId)
	if !ok {
		klog.Errorf("Deleting portmirror %s/%s which was not created succesfully", portMirror.Namespace, portMirror.Name)
		return nil
	}
	...
	if len(portMirrList) == 0 {
		bnnc.mirrorIDToPortMirrorMap.Delete(pm.SinkLocalDetails.PortMirrorId)
	} else {
		// store the updated portmirror list
		bnnc.mirrorIDToPortMirrorMap.Store(pm.SinkLocalDetails.PortMirrorId, portMirrList)
	}
...
func (bnnc *BaseNodeNetworkController) syncPortMirrorsDPUPeriodic() {
	...
	portMirrors, err := bnnc.watchFactory.GetPortMirrors()
	if err != nil {
	...
	for _, sfInterface := range sfInterfaceList {
		...
		if _, ok := portMirrorIDMap[mirrorID]; !ok {
			// get the SF details
			s, ok := bnnc.portMirrorIDToSFMap.Load(mirrorID)
			if !ok {
			...
			bnnc.portMirrorIDToSFMap.Delete(mirrorID)
			// if present, delete mirrorID key from mirrorIDToPortMirrorMap
			if _, ok := bnnc.mirrorIDToPortMirrorMap.Load(mirrorID); ok {
				bnnc.mirrorIDToPortMirrorMap.Delete(mirrorID)
			}
			err := bnnc.unConfigureSFInterface(sfInfo)
			if err != nil {
				klog.Errorf(err.Error())
...
func (bnnc *BaseNodeNetworkController) retryPortMirrorOperationsDPU() bool {
	item, quit := bnnc.portMirrorRetryQueueDPU.Get()
	if quit {
		return false
	}
	bnnc.portMirrorRetryQueueDPU.Done(item)
	...
	if _, err := bnnc.watchFactory.GetPortMirror(retry.pm.Namespace, retry.pm.Name); err != nil {
		if errors.IsNotFound(err) {
			...
			// requeue
			bnnc.requeueSinkPodAddForPortMirror(retry.pm, retry.pod)
		}
		return true
	}
	...
	err := bnnc.handlePortMirrorSinkPodAdd(retry.pm, retry.pod)
	if err != nil {
...
func (bnnc *BaseNodeNetworkController) requeueSinkPodAddForPortMirror(portmirror *util.PortMirror, pod *kapi.Pod) {
	...
	bnnc.portMirrorRetryQueueDPU.AddAfter(req, 3*time.Second)
}
