...
var _ = ginkgo.Describe("VRF manager", func() {
	...
	ginkgo.Context("VRFs", func() {
		...
		ginkgo.It("migrates the slave interface routes into the VRF table on enslavement", func() {
			...
			gomega.Expect(restored).To(gomega.Equal([]*netlink.Route{&expectedHostRoute, &expectedDefaultRoute}),
				"expected the gateway host route to be restored before the default route that depends on it")
		})
