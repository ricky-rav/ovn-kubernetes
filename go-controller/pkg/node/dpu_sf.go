...
func (n *OvnNode) getNextAvailableSFPortNum(startSFNum uint32, reqSFNum int) uint32 {
	n.sfPortNumberMutex.Lock()
	defer n.sfPortNumberMutex.Unlock()

	if reqSFNum >= 0 {
		if _, ok := n.usedSFPortNumMap[uint32(reqSFNum)]; !ok {
			n.usedSFPortNumMap[uint32(reqSFNum)] = true
	...
	for i = startSFNum; i < uint32(MaxSFNum); i++ {
		if _, ok := n.usedSFPortNumMap[i]; !ok {
			n.usedSFPortNumMap[i] = true
			return i
		}
	}
	return i
...
func (n *OvnNode) releaseSFPortNum(sfNum uint32) {
	n.sfPortNumberMutex.Lock()
	defer n.sfPortNumberMutex.Unlock()

	delete(n.usedSFPortNumMap, sfNum)
}

func (n *OvnNode) setSFPortNum(sfNum uint32) {
	n.sfPortNumberMutex.Lock()
	defer n.sfPortNumberMutex.Unlock()

	n.usedSFPortNumMap[sfNum] = true
}
...
func (n *OvnNode) createSF(pciAddress string, pfNum uint16, sfNum uint32) (*mlxdevm.DevlinkPort, error) {
...
func (n *OvnNode) deleteSF(sfUplinkPort string, portIndex, sfNum uint32) error {
...
func (n *OvnNode) createSFWithRetry(uplinkPCIAddress string, sfUplinkPortNum uint16, reqSFNum int) (*mlxdevm.DevlinkPort, uint32, error) {
	var startSFNum uint32 = 0
	for {
		// get the next available SF port number
		mirrorSFNum := n.getNextAvailableSFPortNum(startSFNum, reqSFNum)
		...
		dlPort, err := n.createSF(uplinkPCIAddress, sfUplinkPortNum, mirrorSFNum)
		if err != nil {
			...
			if strings.Contains(err.Error(), "file exists") {
				...
				if reqSFNum >= 0 {
					n.releaseSFPortNum(mirrorSFNum)
...
func (n *OvnNode) getSFInfo(sfUplinkPort string, sfUplinkPortNum uint16, reqSFNum int) (*sfDetails, error) {
	uplinkPCIAddress, err := getPCIFromDeviceName(sfUplinkPort)
	if err != nil {
	...
	dlPort, mirrorSFNum, err := n.createSFWithRetry(uplinkPCIAddress, sfUplinkPortNum, reqSFNum)
	if err != nil {
		return nil, err
	}
	...
	if err := wait.PollImmediate(500*time.Millisecond, 5*time.Second, func() (bool, error) {
		...
		_ = n.deleteSF(sfUplinkPort, dlPort.PortIndex, mirrorSFNum)
