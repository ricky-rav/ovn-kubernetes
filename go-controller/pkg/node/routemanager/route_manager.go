...
// existing one, so flags the kernel attaches on its own (RTNH_F_LINKDOWN on
// a next hop whose device lost carrier) do not count as a difference. The
// weight is compared exactly: Hops is the weight minus one, so zero is a
// requested value, not an unset one.
