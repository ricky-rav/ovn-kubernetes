...
var _ = Describe("Node IP Handler tests", func() {
	...
	BeforeEach(func() {
		...
		node, err = tc.watchFactory.GetNode(nodeName)
		Expect(err).NotTo(HaveOccurred())
		...
		tc.ipManager, err = newAddressManagerInternal(nodeName, k, fakeMgmtPortConfig, tc.watchFactory, fakeBridgeConfiguration, false)
		Expect(err).NotTo(HaveOccurred())
