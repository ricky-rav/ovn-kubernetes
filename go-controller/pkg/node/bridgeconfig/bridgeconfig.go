...
func NewBridgeConfiguration(ovsClient libovsdbclient.Client, intfName, nodeName,
	...
	if len(gwIPs) > 0 {
		...
		// If IP does not exist, error out.
		res.ips, err = nodeutil.GetNetworkInterfaceIPAddresses(gwIntf, nil)
		if err != nil {
...
func (b *BridgeConfiguration) UpdateInterfaceIPAddresses(node *corev1.Node) ([]*net.IPNet, error) {
	...
	ifAddrs, err := nodeutil.GetNetworkInterfaceIPAddresses(b.GetGatewayIface(), b.gwNextHops)
	if err != nil {
		return nil, err
	}
...
func bridgedGatewayNodeSetup(ovsClient libovsdbclient.Client, nodeName, bridgeName, physicalNetworkName string) (string, error) {
	...
	if !strings.Contains(existing, physNetBridgeMapping) {
		// skip the existing mapping setting for the specified physicalNetworkName
		mapString := ""
		bridgeMappings := strings.Split(existing, ",")
