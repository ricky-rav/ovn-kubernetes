...
type BaseNodeNetworkController struct {
	...
	// TBD: use syncmap.SyncMap instead of LockByKey()?
	podNADToDPUCDMap sync.Map

	// per-NAD NADConfig mapping, key is nadName, value is pointer of NADConfig
	NADConfigMap       sync.Map
	totalMaxNewConnPPS uint // if totalMaxNewConnPPS is greater than 0, we'd need to enable DOS Checker
	...
	// chan to sop rate limiting go routine, it is also used to indicate if rate limiting is enabled or not
	DoSCheckStopChan chan struct{}

	// stopChan and WaitGroup per controller
	stopChan chan struct{}
	wg       *sync.WaitGroup

	nadConfigController *nadconfig.NadConfigController
	portMirrorInfo
}

func newCommonNodeNetworkControllerInfo(kubeClient clientset.Interface, kube kube.InterfaceOVN,
	wf factory.NodeWatchFactory, eventRecorder record.EventRecorder, name, dpuName, hostType string,
	pfMACs []string, routeManager *routemanager.Controller) *CommonNodeNetworkControllerInfo {
...
func NewCommonNodeNetworkControllerInfo(ovnNodeClient *util.OVNNodeClientset, wf factory.NodeWatchFactory, eventRecorder record.EventRecorder,
	routeManager *routemanager.Controller, name, dpuName, hostType string, pfMACs []string) *CommonNodeNetworkControllerInfo {
	k := &kube.KubeOVN{
		Kube: kube.Kube{
...
func newDefaultNodeNetworkController(cnnci *CommonNodeNetworkControllerInfo, stopChan chan struct{},
	wg *sync.WaitGroup, routeManager *routemanager.Controller, ovsClient client.Client, networkManager networkmanager.Interface) *DefaultNodeNetworkController {
...
func NewDefaultNodeNetworkController(cnnci *CommonNodeNetworkControllerInfo, networkManager networkmanager.Interface, ovsClient client.Client) (*DefaultNodeNetworkController, error) {
	...
	wg := &sync.WaitGroup{}
	nc := newDefaultNodeNetworkController(cnnci, stopChan, wg, cnnci.routeManager, ovsClient, networkManager)
...
func getMgmtPortAndRepNameModeDPU(node *corev1.Node) (string, string, error) {
	if config.OvnKubeNode.MgmtPortNetdev == "" {
		...
		rep, err := getManagementPortNetDev(config.OvnKubeNode.MgmtPortNetdev)
		if err != nil {
			return "", "", err
		}
		return "", rep, nil
...
func (nc *DefaultNodeNetworkController) Init(ctx context.Context) error {
	klog.Infof("Initializing the default node network controller, Mode %s", config.OvnKubeNode.Mode)
	...
	nc.nodePrimaryIP = nodeAddrStr
	nodeAddr := utilnet.ParseIPSloppy(nodeAddrStr)
	if nodeAddr == nil {
	...
	if config.OvnKubeNode.Mode != types.NodeModeDPU {
		...
		cniServer, err = cni.NewCNIServer(nc.watchFactory, kube.KClient, nc.networkManager, nc.ovsClient)
		if err != nil {
			return err
		}
	...
	if newEndpointSlice != nil {
		for _, newPort := range newEndpointSlice.Ports {
			for _, newEndpoint := range newEndpointSlice.Endpoints {
				for _, newIP := range newEndpoint.Addresses {
					newIPStr := utilnet.ParseIPSloppy(newIP).String()
					if newIPStr != nc.nodePrimaryIP {
						continue
					}
					...
					klog.V(5).Infof("Adding the endpoint that is not present in old slice %s/%d/%s",
						newIPStr, *newPort.Port, *newPort.Protocol)
					err := addPortToFirewallZone(ovnFirewallZone, *newPort.Port, *newPort.Protocol)
					if err != nil {
			...
			for _, oldEndpoint := range oldEndpointSlice.Endpoints {
				for _, oldIP := range oldEndpoint.Addresses {
					...
					klog.Infof("Removing the endpoint %s/%d/%s not present in new slice but present in old slice",
						oldIPStr, *oldPort.Port, *oldPort.Protocol)
					err := removePortFromFirewallZone(ovnFirewallZone, *oldPort.Port, *oldPort.Protocol)
					if err != nil {
