...
// DeleteLogicalSwitchOps return txn ops to delete the provided logical switch
func DeleteLogicalSwitchOps(nbClient libovsdbclient.Client, ops []libovsdb.Operation, swName string) ([]libovsdb.Operation, error) {
...
// updating existing
func UpdateLogicalSwitchSetExternalIDs(nbClient libovsdbclient.Client, sw *nbdb.LogicalSwitch) error {
	externalIds := sw.ExternalIDs
	sw, err := GetLogicalSwitch(nbClient, sw)
	if err != nil {
		return err
	}

	if sw.ExternalIDs == nil {
		sw.ExternalIDs = map[string]string{}
	}

	for k, v := range externalIds {
		if v == "" {
			delete(sw.ExternalIDs, k)
		} else {
			sw.ExternalIDs[k] = v
		}
	}
	...
	opModel := operationModel{
		Model:          sw,
		OnModelUpdates: []interface{}{&sw.ExternalIDs},
		ErrNotFound:    true,
		BulkOp:         false,
	}
...
func DeleteLogicalSwitchPortsWithPredicateOps(nbClient libovsdbclient.Client, ops []libovsdb.Operation, sw *nbdb.LogicalSwitch, p logicalSwitchPortPredicate) ([]libovsdb.Operation, error) {
	lsps, err := FindLogicalSwitchPortsWithPredicate(nbClient, sw, p)
	if err != nil {
		return nil, fmt.Errorf("error finding logical switch ports on switch %+v: %v", sw, err)
	}
