...
// FindRoutersWithPredicate looks up all routers in the cache based on a given predicate
func FindRoutersWithPredicate(nbClient libovsdbclient.Client, p logicalRouterStaticRoutePredicate) ([]nbdb.LogicalRouter, error) {
	ctx, cancel := context.WithTimeout(context.Background(), types.OVSDBTimeout)
	defer cancel()
	found := []nbdb.LogicalRouter{}
	err := nbClient.WhereCache(p).List(ctx, &found)
