...
func getKubeClient(nodeCount int) *kube.Kube {
	...
	return &kube.Kube{fakeClient.KubeClient, nil, nil, nil, nil, nil, nil}
}
...
var _ = ginkgo.Describe("Config Duration Operations", func() {
	...
	ginkgo.Context("Runtime", func() {
		ginkgo.It("records correctly", func() {
			...
			startTimestamp, ok := instance.Start("pod", testNamespaceA, testPodNameA, util.NetNameInfo{NetName: "", Prefix: "", IsSecondary: false})
			gomega.Expect(ok).To(gomega.BeTrue())
			gomega.Expect(startTimestamp.IsZero()).Should(gomega.BeFalse())
			endTimestamp := instance.End("pod", testNamespaceA, testPodNameA, util.NetNameInfo{NetName: "", Prefix: "", IsSecondary: false})
		...
		ginkgo.It("records correctly with OVN latency", func() {
			...
			gomega.Expect(ok).To(gomega.BeTrue())
			gomega.Expect(startTimestamp.IsZero()).Should(gomega.BeFalse())
			ops, txOkCallback, startOVNTimestamp, err := instance.AddOVN(nbClient, "pod", testNamespaceA, testPodNameA, util.NetNameInfo{NetName: "", Prefix: "", IsSecondary: false})
		...
		ginkgo.It("records multiple different objs including adding OVN latency", func() {
			...
			startTimestamp, ok = instance.Start("networkpolicy", testNamespaceB, testPodNameB, util.NetNameInfo{NetName: "", Prefix: "", IsSecondary: false})
			gomega.Expect(ok).To(gomega.BeTrue())
			gomega.Expect(startTimestamp.IsZero()).Should(gomega.BeFalse())
			ops, txOkCallback, startOVNTimestamp, err = instance.AddOVN(nbClient, "networkpolicy", testNamespaceB, testPodNameB, util.NetNameInfo{NetName: "", Prefix: "", IsSecondary: false})
			gomega.Expect(ops).Should(gomega.HaveLen(1))
			gomega.Expect(err).Should(gomega.BeNil())
			_, err = libovsdbops.TransactAndCheck(nbClient, ops)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			txOkCallback()
			endTimestamp = instance.End("networkpolicy", testNamespaceB, testPodNameB, util.NetNameInfo{NetName: "", Prefix: "", IsSecondary: false})
			endOVNTimestamp = startOVNTimestamp.Add(10 * time.Second)
		...
		ginkgo.It("denies recording when no start called", func() {
			instance.Run(nbClient, k, 0, time.Millisecond, stop)
			ops, _, _, _ := instance.AddOVN(nbClient, "pod", testNamespaceA, testPodNameA, util.NetNameInfo{NetName: "", Prefix: "", IsSecondary: false})
		...
		ginkgo.It("allows multiple addOVN records for the same obj", func() {
			...
			gomega.Expect(startTimestamp.IsZero()).Should(gomega.BeFalse())
			// first addOVN
			ops, txOkCallback, firstStartOVNTimestamp, err := instance.AddOVN(nbClient, "pod", testNamespaceA, testPodNameA, util.NetNameInfo{NetName: "", Prefix: "", IsSecondary: false})
			gomega.Expect(ops).Should(gomega.HaveLen(1))
			gomega.Expect(err).Should(gomega.BeNil())
			_, err = libovsdbops.TransactAndCheck(nbClient, ops)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			txOkCallback()
			// second addOVN
			ops, txOkCallback, secondStartOVNTimestamp, err := instance.AddOVN(nbClient, "pod", testNamespaceA, testPodNameA, util.NetNameInfo{NetName: "", Prefix: "", IsSecondary: false})
			gomega.Expect(ops).Should(gomega.HaveLen(1))
			gomega.Expect(err).Should(gomega.BeNil())
			_, err = libovsdbops.TransactAndCheck(nbClient, ops)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
