...
// registry
func RegisterMasterMetrics(nbClient libovsdbclient.Client, sbClient libovsdbclient.Client, metricsScrapeInterval int, stopChan chan struct{}) {
	registerMasterMetricsOnce.Do(func() {
		RegisterMasterBase(metricsScrapeInterval, stopChan)
	})
}
...
func StartMasterMetricUpdater(stopChan <-chan struct{}, sbClient, nbClient libovsdbclient.Client) {
	startMasterMetricUpdaterOnce.Do(func() {
		RunTimestamp(stopChan, sbClient, nbClient)
		MonitorIPSec(nbClient)
	})
}

// RegisterMasterBase registers ovnkube master base metrics with the Prometheus registry.
// This function should only be called once.
func RegisterMasterBase(metricsScrapeInterval int, stopChan chan struct{}) {
	...
	prometheus.MustRegister(metricOvnKubeMasterLogFileSize)                                              //nv
	go ovnKubeLogFileSizeMetricsUpdater(metricOvnKubeMasterLogFileSize, metricsScrapeInterval, stopChan) //nv
}
