...
// newOVNNodeController returns a node handler that listens for node events.
...
func (n *NodeController) hybridOverlayNodeUpdate(node *kapi.Node) error {
	...
	if len(config.HybridOverlay.ClusterSubnets) == 0 {
		...
		mgmtPortLink, err := util.GetNetLinkOps().LinkByName(config.OvnKubeNode.MgmtPortIntfName)
		if err != nil {
			return fmt.Errorf("failed to lookup link %s: %v", config.OvnKubeNode.MgmtPortIntfName, err)
		}
