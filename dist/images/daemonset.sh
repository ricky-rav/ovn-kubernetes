...
OVN_DISABLE_SNAT_MULTIPLE_GWS="false"
...
OVN_EGRESSIP_HEALTHCHECK_PORT="0"
OVN_EGRESSIP_REACHABILITY_TIMEOUT=
...
while [ "$1" != "" ]; do
  ...
  --egress-ip-healthcheck-port)
    ...
    ;;
  --egress-ip-reachability-timeout)
    OVN_EGRESSIP_REACHABILITY_TIMEOUT=$VALUE
    ;;
  ...
  --network-segmentation-enable)
    ...
    ;;
  --network-connect-enable)
    OVN_NETWORK_CONNECT_ENABLE=$VALUE
    ;;
...
ovn_egress_ip_reachability_timeout=${OVN_EGRESSIP_REACHABILITY_TIMEOUT}
echo "ovn_egress_ip_reachability_timeout: ${ovn_egress_ip_reachability_timeout}"
...
echo "ovn_network_connect_enable: ${ovn_network_connect_enable}"
...
cp ../templates/ovnkube-alerts-shardable-common.yaml.j2 ${output_dir}/ovnkube-alerts-shardable-common.yaml
cp ../templates/ovnkube-alerts-shardable-dpu.yaml.j2 ${output_dir}/ovnkube-alerts-shardable-dpu.yaml
cp ../templates/ovnkube-alerts-shardable-cx.yaml.j2 ${output_dir}/ovnkube-alerts-shardable-cx.yaml
